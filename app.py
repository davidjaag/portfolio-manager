...
# ══════════════════════════════════════════════════════════════════════════
def _get_rf(override: float = None) -> float:
    """
    Tasa libre de riesgo.
    - Si override != None, usa ese valor directamente (admin sidebar).
    - Si YF disponible, intenta obtener ^TNX en tiempo real.
    - Fallback: 0.0264 (2.64% igual que MATLAB original).
    El valor usado siempre se muestra en el debug bar.
    """
    ...
    try:
        if YF_AVAILABLE:
            ...
            if not rf_data.empty:
                v = float(rf_data.iloc[-1]) / 100.0
                if 0.001 < v < 0.20:   # sanity check: entre 0.1% y 20%
                    return v
    except Exception:
        pass
    return 0.0264   # fallback = mismo que MATLAB original
...
def core_model_run(prices_df: pd.DataFrame,
    ...
    try:
        # ── RF: override del admin > ^TNX en tiempo real > fallback MATLAB ──
        if rf is not None:
            pass   # ya viene con valor
        else:
            rf = _get_rf(override=rf_override)
...
with st.sidebar:
    ...
    rf_override = 0.0   # siempre automático desde ^TNX
...
@st.cache_data(ttl=300)
def run_model(sid: str, pname: str, rf_override: float = 0.0) -> dict:
    """
    Corre el modelo. RF siempre automático desde ^TNX.
    rf_override se ignora (queda por compatibilidad de firma con cache).
    """
    df = load_prices(sid, pname)
    if df.empty: return {}
    return core_model_run(df, rf_override=None, max_w=0.30)
...
if selected:
    ...
    if use_sheets and sheet_id and not prices_hist.empty:
        model = run_model(sheet_id, selected, rf_override=rf_override)
    ...
    if sheet_id:
        ...
        _rows_validas = len(prices_hist)
        _fecha_ini = _fecha_fin = ""
        if not prices_hist.empty and "__fecha__" in prices_hist.columns:
            _flist = prices_hist["__fecha__"].dropna().tolist()
            if _flist: _fecha_ini, _fecha_fin = str(_flist[0]), str(_flist[-1])
        _n_obs = model.get("n_obs",0) if model and not model.get("error") else 0

        # Leer raw para ver filas totales y tickers con datos incompletos
        _raw_df     = load_prices_raw(sheet_id, selected) if use_sheets else pd.DataFrame()
        _rows_raw   = len(_raw_df)
        _rows_drop  = _rows_raw - _rows_validas
        _missing_info = []
        if not _raw_df.empty and _rows_drop > 0:
            for _tc in [c for c in _raw_df.columns if not c.startswith("__")]:
                _n_nan = int(_raw_df[_tc].isna().sum())
                if _n_nan > 0:
                    _missing_info.append(f"<b>{_tc}</b>: {_n_nan} filas sin precio")

        if prices_hist.empty:
            _diag_cls = "diag-err"
            _diag_msg = f"❌ No se encontró la hoja <b>Precios_{selected}</b>."
        elif _rows_validas < 13:
            _diag_cls = "diag-warn"
            _diag_msg = (f"⚠ Solo <b>{_rows_validas} filas válidas</b> (mínimo 13). "
                        f"Total en hoja: {_rows_raw}.")
        elif _rows_drop > 0:
            _diag_cls = "diag-warn"
            _diag_msg = (
                f"⚠ <b>Precios_{selected}</b> · "
                f"Sheet: <b>{_rows_raw} filas totales</b> · "
                f"Modelo usa: <b>{_rows_validas} filas</b> · "
                f"<b style='color:#ef4444;'>{_rows_drop} filas descartadas</b> "
                f"(tickers sin precio en esas fechas)<br>"
                f"<span style='font-size:11px;'>{'  ·  '.join(_missing_info)}</span><br>"
                f"<b>Solución:</b> Todos los tickers deben tener precio desde la misma "
                f"fecha inicial. Período actual válido: <b>{_fecha_ini} → {_fecha_fin}</b>")
        else:
            _diag_cls = "diag-ok"
            _diag_msg = (
                f"✅ Precios_{selected} · <b>{_rows_validas} filas</b> · "
                f"<b>{_n_obs} retornos mensuales</b> · "
                f"{_fecha_ini} → {_fecha_fin} · "
                f"Activos: <b>{', '.join(_price_cols)}</b>")
        st.markdown(f"<div class='diag {_diag_cls}'>{_diag_msg}</div>",
                   unsafe_allow_html=True)
    else:
        st.markdown("<div class='diag diag-warn'>"
                   "⚠ Sheet no configurado.</div>",
                   unsafe_allow_html=True)
    ...
    # ── DEBUG BAR (admin) ─────────────────────────────────────────────────────
    if is_a:
        _rf_usado = model.get("rf", 0.0) if model and not model.get("error") else 0.0

        # ── Diagnóstico de datos ──────────────────────────────────────────────
        _pcols = [c for c in prices_hist.columns if not c.startswith("__")]
        _diag_rows = ""
        _split_warnings = []

        if not prices_hist.empty and len(_pcols) >= 1 and model and not model.get("error"):
            _first = prices_hist.iloc[0]
            _last  = prices_hist.iloc[-1]
            _fecha_primera = prices_hist["__fecha__"].iloc[0] if "__fecha__" in prices_hist.columns else "—"
            _fecha_ultima  = prices_hist["__fecha__"].iloc[-1] if "__fecha__" in prices_hist.columns else "—"
            _first_vals = " | ".join(f"{c}={_first[c]:.2f}" for c in _pcols[:6])
            _last_vals  = " | ".join(f"{c}={_last[c]:.2f}" for c in _pcols[:6])

            # Detectar retornos acumulados anómalos (posible mezcla pre/post split)
            mu_ann = model.get("mu_ann", {})
            for t, mu_v in mu_ann.items():
                if t.startswith("__"): continue
                # Retorno anual < -30% es sospechoso para un activo en un período de 5+ años
                if mu_v < -0.30:
                    _split_warnings.append(
                        f"⚠ <b>{t}</b>: retorno anual calculado = {mu_v*100:.1f}% "
                        f"(¿datos split no ajustados?)")
                # Precio inicial >> precio final también es sospechoso
                if t in prices_hist.columns:
                    p0 = float(_first[t]) if t in _first else 0
                    p1 = float(_last[t])  if t in _last  else 0
                    if p0 > 0 and p1 > 0 and p0 > p1 * 3:
                        _split_warnings.append(
                            f"⚠ <b>{t}</b>: precio inicial {p0:.2f} >> precio final {p1:.2f} "
                            f"— posible mezcla pre/post split")

            _diag_rows = (
                f"<br><span style='color:#86efac;'>1ª fila [{_fecha_primera}]: {_first_vals}</span>"
                f"<br><span style='color:#fde68a;'>Última [{_fecha_ultima}]: {_last_vals}</span>")
        ...
        st.markdown(
            ...
            f"n_meses: <b>{rm['n']}</b> · "
            f"<span style='color:#facc15;font-weight:700;'>RF: {_rf_usado*100:.4f}%</span>"
            f"{_diag_rows}"
            f"</div>",
            unsafe_allow_html=True)

        # Alertas de datos inconsistentes (split, errores de precio)
        if _split_warnings:
            st.markdown(
                f"<div class='diag diag-err'>"
                f"<b>⚠ Problema de datos detectado en Precios_{selected}:</b><br>"
                + "<br>".join(_split_warnings)
                + "<br><br><b>Solución:</b> El Sheet debe tener precios <b>ajustados por splits</b> "
                f"en TODAS las fechas. Descarga los datos históricos ajustados (Adjusted Close) "
                f"desde Yahoo Finance o Bloomberg y actualiza la hoja <code>Precios_{selected}</code>."
                f"</div>",
                unsafe_allow_html=True)
...
if page in ("📊 Dashboard","📊 Mi Portafolio") and selected:
    ...
    if rm["source"] == "sin_datos":
        st.info(f"📊 **{selected}** — Conecta la hoja **Precios_{selected}** en tu Sheet "
                f"con al menos 13 filas de precios históricos mensuales.")

    # ══════════════════════════════════════════════════════════════════════
    # DOS TABS: MODELO (Excel) vs ACTUAL (Yahoo Finance)
    # ══════════════════════════════════════════════════════════════════════
    tab_model, tab_live = st.tabs([
        "📊 Modelo — datos del Excel histórico",
        "📡 Actual — Yahoo Finance en tiempo real"
    ])

    # ──────────────────────────────────────────────────────────────────────
    # TAB 1: MODELO — todo viene de core_model_run(Excel)
    # Estos datos NO cambian con los precios de hoy — reflejan el análisis
    # histórico de Markowitz sobre el universo del portafolio.
    # ──────────────────────────────────────────────────────────────────────
    with tab_model:
        _sharpe = rm["sharpe"]
        _vol    = rm["vol"]
        ...
        _var95  = rm["var95"]
        _n      = rm["n"]
        _rf_m   = model.get("rf", 0.0) if model and not model.get("error") else 0.0

        # Header de fuente
        _src_color = "#059669" if rm["source"]=="modelo" else "#d97706"
        _src_txt   = (f"📊 Calculado desde <b>Precios_{selected}</b> · "
                      f"<b>{_n} retornos mensuales</b> · RF: <b>{_rf_m*100:.2f}%</b>"
                      if rm["source"]=="modelo" else
                      "⚠ Sin datos de Excel — conecta la hoja Precios_{selected}")
        st.markdown(
            f"<div style='background:#f0fdf4;border-left:4px solid {_src_color};"
            f"border-radius:8px;padding:8px 14px;font-size:12px;margin-bottom:14px;'>"
            f"{_src_txt}</div>",
            unsafe_allow_html=True)

        # ── 4 métricas del modelo ──────────────────────────────────────────
        m1,m2,m3,m4 = st.columns(4)
        _sr_color = "#059669" if _sharpe>=1.0 else ("#d97706" if _sharpe>=0.5 else "#ef4444")
        _sr_txt   = "Excelente" if _sharpe>=1.0 else ("Aceptable" if _sharpe>=0.5 else "Débil")
        with m1:
        ...
        with m3:
            st.markdown(f"""<div class='card'>
                ...
                <div class='kpi-val' style='color:#d97706;'>{_vol*100:.2f}%</div>
                <div class='kpi-sub'>Riesgo del portafolio</div>
            </div>""", unsafe_allow_html=True)
        with m4:
            if model and not model.get("error"):
                _p5  = model.get("mc_p5",  0) * 100
                _p95 = model.get("mc_p95", 0) * 100
                st.markdown(f"""<div class='card'>
                    <div class='kpi-lbl'>Intervalo 90% (1 año)</div>
                    <div class='kpi-val' style='font-size:18px;color:#1a3a5c;'>
                        {_p5:.1f}% – {_p95:.1f}%
                    </div>
                    <div class='kpi-sub'>Escenario pesimista / optimista</div>
                </div>""", unsafe_allow_html=True)
            else:
                st.markdown(f"""<div class='card'>
                    <div class='kpi-lbl'>Intervalo 90% (1 año)</div>
                    <div class='kpi-val' style='font-size:18px;color:#9ca3af;'>—</div>
                    <div class='kpi-sub'>Sin datos de modelo</div>
                </div>""", unsafe_allow_html=True)

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

        # ── Semáforos del modelo ───────────────────────────────────────────
        ...
        sem(s1,
            "🟢" if _sharpe>=1.0 else ("🟡" if _sharpe>=0.5 else "🔴"),
            "Eficiencia (Sharpe)", f"{_sharpe:.3f}",
            "Excelente" if _sharpe>=1.0 else ("Aceptable" if _sharpe>=0.5 else "Débil"),
            _sr_color)
        sem(s2,
            "🟢" if _vol<=_vol_ref*1.3 else "🟡",
            "Volatilidad", f"{_vol*100:.1f}%",
            "Controlada" if _vol<=_vol_ref*1.3 else f"Alta vs mín.var ({_vol_ref*100:.1f}%)",
            "#059669" if _vol<=_vol_ref*1.3 else "#d97706")
        sem(s3,
            "🟢" if _dd_pct<10 else ("🟡" if _dd_pct<25 else "🔴"),
            "Caída Máxima Histórica", f"{_dd*100:.1f}%",
            "Mínima" if _dd_pct<10 else ("Moderada" if _dd_pct<25 else "Importante"),
            "#059669" if _dd_pct<10 else ("#d97706" if _dd_pct<25 else "#ef4444"))
        sem(s4,
            "🟢" if _var_pct<5 else ("🟡" if _var_pct<10 else "🔴"),
            "VaR 95% Mensual", f"{_var_pct:.2f}%",
            "Bajo" if _var_pct<5 else ("Moderado" if _var_pct<10 else "Elevado"),
            "#059669" if _var_pct<5 else ("#d97706" if _var_pct<10 else "#ef4444"))

        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)

        # ── Composición objetivo (pesos del modelo, w > 0.01) ─────────────
        if target_wts:
            st.markdown(f"<div class='sec'>🎯 Composición Óptima · {selected}</div>",
                       unsafe_allow_html=True)
            st.caption("Pesos calculados por Markowitz desde tu Excel histórico. "
                       "Activos con peso < 1% son descartados por el optimizador.")
            _pal2 = ["#1a3a5c","#c9a227","#059669","#ef4444","#5b4fcf",
                     "#0e7490","#b45309","#6d28d9","#be185d","#0891b2"]
            _cols_w = st.columns(min(len(target_wts), 5))
            for _qi, (_t, _w) in enumerate(sorted(target_wts.items(),
                                                    key=lambda x: -x[1])):
                _c2 = _pal2[_qi % len(_pal2)]
                with _cols_w[_qi % len(_cols_w)]:
                    st.markdown(
                        f"<div style='background:white;border-radius:10px;padding:12px;"
                        f"text-align:center;border-top:4px solid {_c2};"
                        f"box-shadow:0 1px 4px rgba(0,0,0,.06);'>"
                        f"<div style='font-size:13px;font-weight:700;'>{_t}</div>"
                        f"<div style='font-size:22px;font-weight:800;color:{_c2};'>"
                        f"{_w*100:.1f}%</div></div>",
                        unsafe_allow_html=True)

        # ── Frontera eficiente (solo admin) ───────────────────────────────
        if is_a and model and not model.get("error") and model.get("frontier"):
            st.markdown(f"<div class='sec'>🗺️ Frontera Eficiente · {selected}</div>",
                       unsafe_allow_html=True)
            fr = model["frontier"]
            fig_f = go.Figure()
            fig_f.add_trace(go.Scatter(
                x=[p["vol"]*100 for p in fr], y=[p["ret"]*100 for p in fr],
                mode="lines+markers", line=dict(color=port_color, width=2.5),
                marker=dict(size=7), name="Frontera",
                hovertemplate="σ=%{x:.1f}% · R=%{y:.1f}%<extra></extra>"))
            fig_f.add_trace(go.Scatter(
                x=[model["vol_tang"]*100], y=[model["ret_tang"]*100],
                mode="markers",
                marker=dict(size=16, color="#c9a227", symbol="star",
                            line=dict(width=2, color="#0d1b2a")),
                name=f"Tangencia SR={model['sharpe_tang']:.3f}"))
            _rf_p = model["rf"]*100
            _vt   = model["vol_tang"]*100; _rt = model["ret_tang"]*100
            _xc   = [0, max(p["vol"] for p in fr)*110]
            _yc   = [_rf_p, _rf_p+(_rt-_rf_p)/_vt*_xc[1]] if _vt>0 else [_rf_p,_rf_p]
            fig_f.add_trace(go.Scatter(x=_xc, y=_yc, mode="lines",
                line=dict(color="#ef4444", width=1.5, dash="dash"),
                name=f"CML (RF={_rf_p:.2f}%)"))
            fig_f.update_layout(
    ...
    # ──────────────────────────────────────────────────────────────────────
    # TAB 2: ACTUAL — Yahoo Finance + posiciones reales del usuario
    # Estos datos reflejan el valor HORA de lo que el usuario tiene comprado.
    # ──────────────────────────────────────────────────────────────────────
    with tab_live:
        # Header de fuente
        _yf_color = "#059669" if yf_prices else "#d97706"
        _yf_txt   = (f"📡 Precios en tiempo real de Yahoo Finance · "
                     f"<b>{len(yf_prices)} tickers</b> disponibles"
                     if yf_prices else
                     "⚠ Yahoo Finance no disponible — mostrando último precio del Excel")
        st.markdown(
            f"<div style='background:#eff6ff;border-left:4px solid {_yf_color};"
            f"border-radius:8px;padding:8px 14px;font-size:12px;margin-bottom:14px;'>"
            f"{_yf_txt}</div>",
            unsafe_allow_html=True)

        # ── KPIs de posición real ──────────────────────────────────────────
        k1,k2,k3,k4 = st.columns(4)
        with k1:
            st.markdown(f"""<div class='card'>
                <div class='kpi-lbl'>Valor Actual · {selected}</div>
        ...
        with k3:
            _twr_pct = twr.get("twr", None)
            _twr_c = "#059669" if (_twr_pct or 0)>=0 else "#ef4444"
            st.markdown(f"""<div class='card'>
                <div class='kpi-lbl'>Rendimiento Real (TWR)</div>
                <div class='kpi-val' style='color:{_twr_c};'>
                    {f"{_twr_pct:+.2f}%" if _twr_pct is not None else "Sin ops"}</div>
                <div class='kpi-sub'>Desde tu primera operación</div>
            </div>""", unsafe_allow_html=True)
        with k4:
            _n_ops = len(ops_df) if not ops_df.empty else 0
            st.markdown(f"""<div class='card'>
                <div class='kpi-lbl'>Operaciones registradas</div>
                <div class='kpi-val' style='color:#1a3a5c;'>{_n_ops}</div>
                <div class='kpi-sub'>En Ops_{selected}</div>
            </div>""", unsafe_allow_html=True)

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

        # ── Cotizaciones actuales de Yahoo Finance ─────────────────────────
        st.markdown(f"<div class='sec'>📡 Cotizaciones · {selected}</div>",
                   unsafe_allow_html=True)
        _show_tickers = list(target_wts.keys())
        if _show_tickers:
            _all_quotes = get_quotes(_show_tickers)
            _qcols = st.columns(min(len(_show_tickers), 5))
            for _qi, _tkr in enumerate(_show_tickers):
                _q  = _all_quotes.get(_tkr, {})
                _px = _q.get("price")
                # Precio del Excel para comparar
                _px_excel = _last_excel_price.get(_tkr)
                with _qcols[_qi % len(_qcols)]:
                    if not _px and not _px_excel:
                        st.markdown(
                            f"<div class='card' style='text-align:center;'>"
                            f"<b>{_tkr}</b><br>"
                            f"<span style='color:#9ca3af;font-size:12px;'>Sin datos</span>"
                            f"</div>", unsafe_allow_html=True)
                        continue
                    _chg = _q.get("chg", 0) or 0
                    _cc  = "#059669" if _chg>=0 else "#ef4444"
                    _cbg = "#ecfdf5" if _chg>=0 else "#fef2f2"
                    # Diferencia Yahoo vs Excel (muestra si difieren)
                    _diff_note = ""
                    if _px and _px_excel and abs(_px - _px_excel) > 0.01:
                        _diff_pct = (_px/_px_excel - 1)*100
                        _diff_note = (f"<div style='font-size:9px;color:#6b7280;margin-top:4px;'>"
                                     f"vs Excel: {_diff_pct:+.1f}% "
                                     f"(Excel={_px_excel:.2f})</div>")
                    _px_show = _px or _px_excel or 0
                    st.markdown(
                        f"<div class='card'>"
                        ...
                        f"{'▲' if _chg>=0 else '▼'} {abs(_chg):.2f}%</span></div>"
                        f"<div style='font-size:24px;font-weight:700;color:#111827;"
                        f"margin-bottom:4px;'>${_px_show:,.2f}</div>"
                        f"<div style='font-size:9px;color:#6b7280;'>"
                        f"{'📡 Yahoo Finance' if _px else '📊 Excel (sin YF)'}</div>"
                        f"{_diff_note}"
                        f"</div>", unsafe_allow_html=True)

        # ── Alertas de rebalanceo ──────────────────────────────────────────
        if has_act:
            st.markdown(f"<div class='sec'>📋 Acciones · {selected}</div>",
                       unsafe_allow_html=True)
            st.caption("Basado en pesos objetivo del modelo vs tu posición actual.")
            ...
            for i, (_, r) in enumerate(pending.iterrows()):
                with ocols[i % len(ocols)]:
                    if r["Alerta"] == "COMPRAR":
                        ...
                        st.markdown(
                            f"<div style='background:#ecfdf5;border-radius:12px;"
                            f"padding:14px;border-left:4px solid #059669;'>"
                            f"<div style='font-weight:700;font-size:14px;color:#059669;'>"
                            f"🟢 COMPRAR {r['Ticker']}</div>"
                            f"<div style='font-size:12px;margin-top:6px;'>"
                            f"{r['Acciones a Comprar']:.4f} acc @ ${r['Precio Actual']:,.2f}</div>"
                            f"<div style='font-size:12px;color:#059669;font-weight:600;'>"
                            f"${u:,.2f} USD · ${m:,.0f} MXN</div>"
                            f"<div style='font-size:10px;color:#6b7280;margin-top:4px;'>"
                            f"{r['Peso Actual']*100:.1f}% → {r['Peso Objetivo']*100:.1f}%</div>"
                            f"</div>", unsafe_allow_html=True)
                    else:
                        m = r["Monto a Vender MXN"]; u = m/tc_actual
                        st.markdown(
                            f"<div style='background:#fef2f2;border-radius:12px;"
                            f"padding:14px;border-left:4px solid #ef4444;'>"
                            f"<div style='font-weight:700;font-size:14px;color:#ef4444;'>"
                            f"🔴 VENDER {r['Ticker']}</div>"
                            f"<div style='font-size:12px;margin-top:6px;'>"
                            f"{r['Acciones a Vender']:.4f} acc @ ${r['Precio Actual']:,.2f}</div>"
                            f"<div style='font-size:12px;color:#ef4444;font-weight:600;'>"
                            f"${u:,.2f} USD · ${m:,.0f} MXN</div>"
                            f"<div style='font-size:10px;color:#6b7280;margin-top:4px;'>"
                            f"{r['Peso Actual']*100:.1f}% → {r['Peso Objetivo']*100:.1f}%</div>"
                            f"</div>", unsafe_allow_html=True)
        ...
        with pc1:
            if not alerts_df.empty:
                ...
                st.info("Sin posiciones. Registra operaciones en la pestaña Operaciones.")

        with pc2:
            st.markdown(f"<div style='font-size:13px;font-weight:600;margin-bottom:8px;'>"
                       f"Actual vs Objetivo · {selected}</div>", unsafe_allow_html=True)
            _pal  = ["#1a3a5c","#c9a227","#059669","#ef4444",
            ...
            if _has_actual:
                _fig = make_subplots(1,2, specs=[[{"type":"pie"},{"type":"pie"}]],
                ...
                _fig.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)",
                                   margin=dict(l=10,r=10,t=30,b=60),
                                   legend=dict(orientation="h",y=-0.25,
                                               x=0.5,xanchor="center",font_size=10))
