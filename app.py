...
def calc_alerts(positions_df, target_weights, tc, monthly_contrib,
    ...
    if total_mxn == 0:
        ...
        df["Desviacion"]=-df["Peso Objetivo"];df["Alerta"]="OK"
...
if use_sheets and sheet_id:
    ...
    if ops_raw is not None and not ops_raw.empty:
        if "Usuario" in ops_raw.columns:
            _filtered = ops_raw[ops_raw["Usuario"].astype(str).str.lower() == current_user.lower()].copy()
            ops_df = _filtered if not _filtered.empty else ops_raw
        else:
            ops_df = ops_raw
