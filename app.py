...
def core_model_run(prices_df: pd.DataFrame, rf: float = 0.0264,
    ...
    """
    Réplica exacta de Project_R_30_max__1_.mlx de MATLAB.
    Pasos idénticos:
      1. diff(log(Precios))  → rendimientos logarítmicos
      2. mean*12, cov*12     → estadísticos anualizados
      3. estimateMaxSharpeRatio equivalente → portafolio tangencia
         (multi-start + reformulación QP para garantizar óptimo global)
      4. estimateFrontier 20 puntos
      5. SML / betas vs SPX
      6. MC 10,000 sims → VaR, CVaR, intervalos por horizonte
      7. Filtro w > 0.01 para composición (igual que MATLAB pie chart)
    """
    ...
    try:
        ...
        # ── PASO 1: Eliminar filas con NaN (igual que MATLAB filas_validas) ──
        df = df.dropna()

        # ── PASO 2: Identificar SPX — entra al optimizador como activo ───────
        spx_col    = next((c for c in df.columns if "SPX" in str(c).upper()), None)
        asset_cols = list(df.columns)   # TODOS los activos incluyendo SPX
        n          = len(asset_cols)
        if n < 2:
            return {"error": "menos_de_2_activos", "activos": asset_cols}

        # ── PASO 3: Rendimientos logarítmicos  Rendimientos = diff(log(P)) ───
        P = df[asset_cols].values.astype(float)
        R = np.diff(np.log(P), axis=0)   # shape (T, n)
        T = R.shape[0]

        # ── PASO 4: Estadísticos anualizados ─────────────────────────────────
        # AssetMean = mean(Rendimientos,1)' * 12
        # AssetCovar = cov(Rendimientos) * 12
        mu  = R.mean(axis=0) * 12
        cov = np.cov(R.T, ddof=1) * 12

        # ── PASO 5: Portafolio Tangencia — estimateMaxSharpeRatio ────────────
        # MATLAB usa QP paramétrico (Dinkelbach). Replicamos con:
        # (a) Reformulación QP exacta: min y'Σy s.t. (μ-rf)'y=1, y≥0
        # (b) Multi-start SLSQP desde 5 puntos iniciales distintos
        # Nos quedamos con el mejor Sharpe encontrado.
        bnd  = [(0.0, max_w)] * n
        ...
        # Puntos de inicio: uniforme, concentrado en top-Sharpe individual,
        # proporcional a retornos en exceso, aleatorios
        rng = np.random.default_rng(42)
        excess = mu - rf
        starts = [np.ones(n) / n]                          # uniforme
        if (excess > 0).any():
            pos = np.clip(excess, 0, None)
            starts.append(pos / pos.sum())                 # prop. retorno exceso
        ratios = np.where(np.diag(cov) > 0,
                          excess / np.sqrt(np.diag(cov)), -np.inf)
        best_i = int(np.argmax(ratios))
        w_conc = np.zeros(n); w_conc[best_i] = 1.0
        starts.append(w_conc)                              # concentrado en mejor activo
        for _ in range(7):                                 # 7 aleatorios
            w_rand = rng.dirichlet(np.ones(n))
            w_rand = np.clip(w_rand, 0, max_w)
            if w_rand.sum() > 0:
                w_rand /= w_rand.sum()
            starts.append(w_rand)
        ...
        for w0 in starts:
            ...
            if res.success:
                wc = np.clip(res.x, 0, max_w); wc /= wc.sum()
                sr_c = -neg_sr(wc)
                if sr_c > best_sr:
                    best_sr = sr_c
                    best_wt = wc
        ...
        # ── PASO 6: Mínima Varianza ───────────────────────────────────────────
        def port_var(w): return float(w @ cov @ w)
        best_wv = np.ones(n) / n; best_vv = np.inf
        for w0 in [np.ones(n)/n, wt]:
    ...
    with st.expander("➕ Crear Nuevo Portafolio", expanded=not bool(get_all_ports())):
        ...
        with a1:
            ...
            np_desc  = st.text_input("Descripción breve",
                                      placeholder="Ej: Perfil conservador de renta fija",
                                      key="npd")
        with a2:
            np_color = st.color_picker("Color identificador", value="#3b82f6", key="npc")
            np_rf    = st.number_input("Tasa libre de riesgo (%)",
                                        value=2.64, step=0.01, format="%.2f",
                                        help="Usada por el modelo para calcular Sharpe. Default: 2.64%")

        st.markdown(
            f"<div style='font-size:11px;color:#6b7280;margin-top:4px;'>"
            f"Una vez creado, la app buscará la hoja <b>Precios_{{nombre}}</b> en tu Sheet. "
            f"Los activos, pesos óptimos, Sharpe, volatilidad y retorno se calculan automáticamente "
            f"desde esos datos — no tienes que ingresar nada más.</div>",
            unsafe_allow_html=True)
        ...
        if st.button("➕ Crear Portafolio", key="btn_create", type="primary"):
            ...
            if not n:
                ...
                # Solo guardamos los 3 campos del admin.
                # Todo lo demás (Sharpe, vol, ret, pesos) lo calcula el modelo
                # automáticamente desde Precios_<n> en el Sheet.
                st.session_state.custom_portfolios[n] = {
                    "description": np_desc.strip() or n,
                    "color":       np_color,
                    "border":      np_color,
                    "rf":          np_rf / 100,
                    "target":      {},   # el modelo lo llena
                ...
                st.success(
                    ...
                    f"El modelo correrá automáticamente cuando detecte los datos.")
                st.cache_data.clear()
                st.rerun()
