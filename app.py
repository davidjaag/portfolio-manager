...
# ── IMPORTAR FUNCIÓN DE ANÁLISIS DESDE EL SCRIPT ORIGINAL ───────────────────
from core_markowitz import run_complete_analysis
...
# ── Portfolio Excel/HTML Report Download ──────────────────────────────────────
def build_portfolio_excel(port_name, port_data, positions_df, history,
                          alerts_df, risk_metrics, target_wts, tc):
    """Build a styled Excel report for the portfolio."""
    buf = io.BytesIO()
    try:
        import openpyxl
        ...
        # ── Colors ──
        NAVY   = "0D1B2A"; GOLD = "C9A227"; GREEN = "1A7A4A"
        RED    = "C0392B"; LIGHT = "EDF2F8"; WHITE = "FFFFFF"
        YELLOW = "F9D100"
        def hdr(ws, row, col, val, bg=NAVY, fg=WHITE, sz=11, bold=True, wrap=False):
            ...
            c.alignment = Alignment(horizontal="center", vertical="center",
                                     wrap_text=wrap)
            return c
        def cell(ws, row, col, val, bg=WHITE, fg="1A2332", sz=11,
                 bold=False, align="left", fmt=None):
            c = ws.cell(row=row, column=col, value=val)
            c.font = Font(name="Calibri", bold=bold, color=fg, size=sz)
            c.fill = PatternFill("solid", fgColor=bg)
        ...
        hdr(ws1, 1, 1, f"EGADE · Portfolio Manager — {port_name}", NAVY, GOLD, 14, True)
        ws1.merge_cells("A2:E2")
        hdr(ws1, 2, 1, f"Reporte Ejecutivo · {datetime.now().strftime("%d %b %Y %H:%M")}",
            "1A3A5C", WHITE, 10, False)
        # Risk KPIs
        row = 4
        for label, key, fmt_s in [
            ("Sharpe Ratio",      "sharpe",      "0.00##"),
            ("Retorno Anual",     "ret_annual",  "0.00%"),
            ("Volatilidad Anual", "vol_annual",  "0.00%"),
            ("VaR 95%",           "var_95",      "0.00%"),
            ("CVaR 95%",          "cvar_95",     "0.00%"),
        ]:
            val = risk_metrics.get(key, 0)
            if key in ("ret_annual","vol_annual","var_95","cvar_95"): val = abs(val)
            hdr(ws1, row, 1, label, "E8F0F8", "1A2332", 10, True)
            c = cell(ws1, row, 2, val, WHITE, GREEN if val>0 else RED, 11, True, "center", fmt_s)
            border_row(ws1, row, 5); row += 1
        # Positions
        row += 1
        ws1.merge_cells(f"A{row}:E{row}")
        hdr(ws1, row, 1, "POSICIONES ACTUALES", NAVY, GOLD, 11); row += 1
        for lbl, col in [("Ticker",1),("Acciones",2),("Precio USD",3),
                           ("Valor MXN",4),("Peso Actual %",5)]:
            hdr(ws1, row, col, lbl, "1A3A5C", WHITE, 10)
        row += 1
        if not positions_df.empty:
            for _, r in positions_df.iterrows():
                bg = "F0FBF4" if str(r.get("Alerta","")) == "COMPRAR" else \
                     "FFF0F0" if str(r.get("Alerta","")) == "VENDER" else WHITE
                cell(ws1, row, 1, str(r.get("Ticker","")), bg, "1A2332", 10, True)
                cell(ws1, row, 2, r.get("Acciones", 0), bg, "1A2332", 10, False, "center", "0.00")
                cell(ws1, row, 3, r.get("Precio USD", 0), bg, "1A2332", 10, False, "center", "$#,##0.00")
                cell(ws1, row, 4, r.get("Valor MXN", 0), bg, "1A2332", 10, False, "center", "$#,##0")
                wpct = r.get("Peso Actual", r.get("w_actual", 0))
                if isinstance(wpct, str): wpct = float(wpct.replace("%",""))/100 if "%" in wpct else 0
                cell(ws1, row, 5, wpct, bg, "1A2332", 10, False, "center", "0.0%")
                border_row(ws1, row, 5); row += 1
        # Alerts
        if not alerts_df.empty:
            row += 1
            ws1.merge_cells(f"A{row}:E{row}")
            hdr(ws1, row, 1, "ALERTAS DE REBALANCEO", YELLOW, "1A1400", 11); row += 1
            for _, ar in alerts_df[alerts_df["Alerta"].isin(["COMPRAR","VENDER"])].iterrows():
                bg = "FFF9C4"
                cell(ws1, row, 1, str(ar.get("Ticker","")), bg, "1A1400", 10, True)
                alerta = str(ar.get("Alerta",""))
                c_color = GREEN if alerta == "COMPRAR" else RED
                cell(ws1, row, 2, alerta, bg, c_color, 10, True, "center")
                cell(ws1, row, 3, str(ar.get("Actual %", ar.get("w_actual",""))), bg, "1A2332", 10, False, "center")
                cell(ws1, row, 4, str(ar.get("Objetivo %", ar.get("w_obj",""))), bg, "1A2332", 10, False, "center")
                border_row(ws1, row, 5); row += 1
        ...
        hdr(ws2, 1, 1, f"Evolución Histórica — {port_name}", NAVY, GOLD, 12)
        hdr(ws2, 2, 1, "Mes", "1A3A5C", WHITE, 10)
        hdr(ws2, 2, 2, "Valor MXN", "1A3A5C", WHITE, 10)
        hdr(ws2, 2, 3, "Retorno %", "1A3A5C", WHITE, 10)
        if not history.empty and "Mes" in history.columns:
            for i, (_, hr) in enumerate(history.iterrows()):
                r3 = i + 3
                cell(ws2, r3, 1, str(hr.get("Mes","")), WHITE, "1A2332", 10)
                cell(ws2, r3, 2, hr.get("Valor",0), WHITE, "1A2332", 10, False, "center", "$#,##0")
                ret_v = hr.get("Retorno",0)
                fg = GREEN if ret_v >= 0 else RED
                cell(ws2, r3, 3, ret_v/100 if abs(ret_v)>1 else ret_v,
                     "F0FBF4" if ret_v>=0 else "FFF0F0", fg, 10, False, "center", "+0.0%;-0.0%")
                border_row(ws2, r3, 3)

        # ════ SHEET 3: PESOS OBJETIVO ════
        ws3 = wb.create_sheet("Pesos Objetivo")
        ws3.sheet_view.showGridLines = False
        ws3.column_dimensions["A"].width = 16
        ws3.column_dimensions["B"].width = 16
        ws3.column_dimensions["C"].width = 40
        ws3.merge_cells("A1:C1")
        hdr(ws3, 1, 1, f"Pesos Objetivo Markowitz — {port_name}", NAVY, GOLD, 12)
        hdr(ws3, 2, 1, "Ticker", "1A3A5C", WHITE, 10)
        hdr(ws3, 2, 2, "Peso %", "1A3A5C", WHITE, 10)
        hdr(ws3, 2, 3, "Descripción", "1A3A5C", WHITE, 10)
        ASSET_DESC = {
            "NVDA":"NVIDIA — Semiconductores e IA generativa",
            "CAT":"Caterpillar — Maquinaria industrial cíclica",
            "NEM":"Newmont — Minería de oro, activo refugio",
            "BATS":"BAT — Tabaco, defensivo de alto dividendo",
            "GLD":"SPDR Gold ETF — Activo refugio físico",
            "IAU":"iShares Gold ETF — Activo refugio",
            "GE":"GE Aerospace — Aviación y defensa",
            "JPM":"JPMorgan — Banca diversificada global",
        }
        for i, (tk, wt) in enumerate(target_wts.items()):
            r4 = i + 3
            bg = "EDF2F8" if i % 2 == 0 else WHITE
            cell(ws3, r4, 1, tk, bg, "1A2332", 10, True)
            cell(ws3, r4, 2, wt, bg, "1A3A5C", 10, False, "center", "0.0%")
            cell(ws3, r4, 3, ASSET_DESC.get(tk, tk), bg, "6B7C93", 10)
            border_row(ws3, r4, 3)
        ...
        if not positions_df.empty:
            buf = io.BytesIO(positions_df.to_csv(index=False).encode())
        else:
...
def send_alert_email(user_name, user_email, portfolio, n_buy, n_sell, alerts_detail):
    """Send portfolio rebalance alert via SMTP.
    Secrets needed under [email]: smtp_host, smtp_port, smtp_user, smtp_pass, from_name
    Add 'email' field to each user in USERS dict to enable notifications.
    """
    try:
        import smtplib
        ...
        if not cfg or not user_email:
            return False
        parts_txt = []
        if n_buy:  parts_txt.append(f"{n_buy} acción(es) a COMPRAR")
        if n_sell: parts_txt.append(f"{n_sell} acción(es) a VENDER")
        subject = f"\u26a0 Alerta {portfolio} \u2014 {chr(32).join(parts_txt)}"
        rows_html = "".join(
            ...
            f"{r.get('Alerta','')}</td>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #f0f4f9;'>{r.get('Actual %',r.get('w_actual',''))}</td>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #f0f4f9;'>{r.get('Objetivo %',r.get('w_obj',''))}</td></tr>"
        ...
        <div style="max-width:560px;margin:32px auto;background:white;border-radius:16px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#0d2240,#1a3a5c);padding:28px 32px;">
            <div style="font-size:22px;color:white;font-weight:700;">EGADE Portfolio Manager</div>
          </div>
          <div style="padding:28px 32px;">
            <div style="background:#f9d100;border-radius:10px;padding:14px 20px;margin-bottom:18px;">
              ...
              <div style="font-size:12px;color:#5c4d00;margin-top:3px;">{portfolio} \u00b7 {" \u00b7 ".join(parts_txt)}</div>
            </div>
            <p>Hola <b>{user_name}</b>,</p>
            <p style="color:#4a5568;font-size:13px;">Tu portafolio <b>{portfolio}</b> requiere rebalanceo:</p>
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
              <thead><tr style="background:#1a3a5c;color:white;">
                <th style="padding:10px 12px;text-align:left;">Activo</th>
                <th style="padding:10px 12px;text-align:left;">Acci\u00f3n</th>
                <th style="padding:10px 12px;text-align:left;">Actual</th>
                <th style="padding:10px 12px;text-align:left;">Objetivo</th>
              </tr></thead>
              <tbody>{rows_html}</tbody>
            </table>
            <p style="color:#6b7c93;font-size:11px;margin-top:18px;">
              {_dt.now().strftime("%d %b %Y %H:%M")} \u00b7 Portfolio Manager EGADE</p>
          </div></div></body></html>"""
        msg = MIMEMultipart("alternative")
        msg["Subject"] = subject
        msg["From"]    = f"{cfg.get('from_name','Portfolio Manager')} <{cfg['smtp_user']}>"
        msg["To"]      = user_email
...
@media(max-width:768px){
    [data-testid="stHeader"]{
        ...
        overflow:visible !important;z-index:9999;
    }
...
.app-banner{
    ...
    box-shadow:0 1px 0 rgba(255,255,255,0.07) inset,0 -2px 0 rgba(0,0,0,0.4) inset,
               0 20px 60px rgba(13,27,42,0.35),0 6px 16px rgba(13,27,42,0.22);
    position:relative;overflow:hidden;flex-wrap:wrap;
}
.app-banner::before{content:'';position:absolute;top:-80%;right:-5%;width:500px;height:500px;border-radius:50%;
    background:radial-gradient(circle,rgba(201,162,39,0.09) 0%,transparent 60%);pointer-events:none;}
.banner-left{flex:1;min-width:0;}
.banner-right{text-align:right;flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;justify-content:space-between;}
.banner-greeting{font-size:18px;color:rgba(255,255,255,0.82);margin-top:8px;font-weight:500;letter-spacing:0.01em;}
.banner-greeting b{color:#c9a227;font-weight:700;}
.banner-title{font-family:'DM Serif Display',serif;font-size:32px;color:#fff;margin:0;letter-spacing:-0.01em;}
...
.banner-count{font-family:'DM Serif Display',serif;font-size:38px;color:#fff;line-height:1;margin-bottom:0;}
.banner-count-label{font-size:12px;color:rgba(232,240,248,0.6);margin-top:0;line-height:1.2;}
...
.table-wrap{
    ...
    border-radius:16px;
    background:linear-gradient(135deg,rgba(255,255,255,0.72) 0%,rgba(232,240,248,0.55) 100%);
    backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);
    border:1px solid rgba(26,58,92,0.13);
    box-shadow:0 8px 32px rgba(13,27,42,0.10), 0 1px 0 rgba(255,255,255,0.8) inset;
    padding:2px;
}
...
@media(max-width:768px){
    .app-banner{padding:18px 18px;border-radius:14px;}
    .banner-title{font-size:22px;}
    .banner-count{font-size:28px;}
    .banner-greeting{font-size:12px;}
    .kpi-card{padding:14px 12px;}
    .kpi-value{font-size:22px;}
    .section-title{font-size:18px;}
    .alert-card{padding:14px 16px;}
    .alert-ticker{font-size:16px;}
...
@media(max-width:480px){
    .app-banner{
        ...
        gap:0;padding:20px 18px 16px;border-radius:16px;
    }
    .banner-left{margin-bottom:0;}
    .banner-title{font-size:28px;}
    .banner-sub{font-size:12px;margin-top:2px;}
    .banner-date{font-size:10px;margin-top:2px;}
    /* divider line between left and right on mobile */
    .banner-right{
        text-align:left;width:100%;display:flex;
        flex-direction:row;align-items:center;justify-content:space-between;
        border-top:1px solid rgba(255,255,255,0.15);margin-top:14px;padding-top:12px;
    }
    .banner-count{font-size:30px;line-height:1;}
    .banner-count-label{font-size:11px;}
    .banner-greeting{font-size:16px;margin-top:0;text-align:right;font-weight:500;}
    .kpi-card{padding:12px 10px;border-radius:10px;}
    .kpi-label{font-size:9px;}
    .kpi-value{font-size:20px;}
    .kpi-sub{font-size:10px;}
    .section-title{font-size:16px;margin:16px 0 10px 0;}
    .alert-card{border-radius:10px;padding:12px 14px;}
    .alert-ticker{font-size:15px;}
    .alert-action{font-size:12px;}
    .table-wrap{border-radius:6px;}
    .styled-table{font-size:11px;min-width:400px;}
    .styled-table th,.styled-table td{padding:7px 8px;}
    div[data-testid="stSidebar"]{min-width:0 !important;}
    /* sidebar toggle via stHeader on mobile */
    div[data-testid="stHorizontalBlock"]{flex-wrap:wrap !important;gap:8px !important;}
    div[data-testid="stHorizontalBlock"] > div{min-width:calc(50% - 4px) !important;}
}
.styled-table{
...
.styled-table th{
    ...
    color:#e8f0f8;padding:12px 14px;text-align:center;
    font-weight:700;font-size:11px;letter-spacing:.08em;
    text-transform:uppercase;white-space:nowrap;
    border-bottom:2px solid rgba(201,162,39,0.45);
}
.styled-table th:first-child{border-radius:13px 0 0 0;}
.styled-table th:last-child{border-radius:0 13px 0 0;}
.styled-table td{
...
.styled-table tr:last-child td:first-child{border-radius:0 0 0 13px;}
.styled-table tr:last-child td:last-child{border-radius:0 0 13px 0;}
.styled-table tr:hover td{background:rgba(26,58,92,0.07);}
...
/* ── CHART GLASSMORPHISM WRAPPER ── */
.chart3d {
    background: linear-gradient(
        145deg,
        rgba(255,255,255,0.78) 0%,
        rgba(232,240,248,0.60) 100%
    );
...
.chart3d::before {
    ...
    background: linear-gradient(90deg,
        rgba(201,162,39,0) 0%,
        rgba(201,162,39,0.55) 40%,
        rgba(201,162,39,0.55) 60%,
...
@keyframes countUp{from{opacity:0;transform:translateY(10px) scale(0.82);}to{opacity:1;transform:translateY(0) scale(1);}}
.kpi-value,.banner-count,.kpi-big,.metric-num,.ret-num,.risk-num{
    animation:countUp 0.6s cubic-bezier(.22,.68,0,1.25) both;
}
.num-anim{animation:countUp 0.55s cubic-bezier(.22,.68,0,1.25) both;display:inline-block;}
...
.kpi-card:nth-child(5) .kpi-value{animation-delay:0.25s;}
@keyframes countUp{from{opacity:0;transform:translateY(8px) scale(0.85);}to{opacity:1;transform:translateY(0) scale(1);}}
.num-anim{animation:countUp 0.6s cubic-bezier(.22,.68,0,1.2) both;display:inline-block;}
...
@media(max-width:768px){.mobile-nav-container{display:block;}}
/* Investor action cards */
.action-card{border-radius:18px;padding:20px 24px;margin:10px 0;
    box-shadow:0 4px 24px rgba(13,27,42,0.12);position:relative;}
.action-buy{background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #16a34a;}
.action-sell{background:linear-gradient(135deg,#fff5f5,#fee2e2);border:2px solid #dc2626;}
.action-ticker{font-family:'DM Serif Display',serif;font-size:26px;color:#1a2332;}
.action-label{font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;}
</style>
...
# ========== NUEVA FUNCIÓN: CARGA DE PRECIOS HISTÓRICOS ==========
@st.cache_data(ttl=300)
def load_prices_history_from_sheets(sheet_id, portfolio_name):
    """
    Lee TODAS las filas de Precios_<portfolio_name> (datos Bloomberg limpios).
    Devuelve pd.DataFrame con índice de fechas y columnas = tickers, valores = precios.
    Si no existe esa hoja, intenta con 'Precios' genérica.
    """
    client = get_gspread_client()
    if client is None:
        return None
    try:
        sh = client.open_by_key(sheet_id)
        # Intentar con la hoja específica del portafolio, luego la genérica
        for tab in [f"Precios_{portfolio_name}", "Precios"]:
            try:
                ws = sh.worksheet(tab)
                data = ws.get_all_values()
                ...
                prices_df = pd.DataFrame(price_rows, index=fechas).dropna(axis=1, how='any')
...
def get_demo_portfolios():
    """Returns base + session custom portfolios."""
    base = _get_base_portfolios()
    custom = st.session_state.get("custom_portfolios", {})
    return {**base, **custom}


def _get_base_portfolios():
    """
...
def get_live_quotes(tickers):
    """Returns dict ticker → {price, open, prev_close, day_high, day_low,
                               change_pct, week52_high, week52_low, ma52w, vs_ma52w_pct}"""
    if not YF_AVAILABLE:
        return {}
    results = {}
    for tkr in tickers:
        ...
        try:
            ...
            # 52-week average = midpoint of 52w range (approx moving average)
        ...
        for i, row in price_df.iterrows():
            ...
            for tkr in price_df.columns:
                ...
                if qty > 0 and price_now > 0:
                    portfolio_value += qty * price_now  # USD
...
def calc_alerts(positions_df, target_weights, tc, monthly_contrib,
    ...
    if total_mxn == 0:
        ...
        df["Alerta"]              = df["Peso Objetivo"].apply(
            lambda w: "COMPRAR" if w > 0 else "OK")
...
if not st.session_state.authenticated:
    ...
    /* sidebar hidden on login via no-content, not CSS */
    ...
    .gpanel{
        ...
        padding:36px 40px; position:relative; overflow:visible; z-index:1;
    ...
    /* Collapse ALL gaps in login form */
    [data-testid="stAppViewContainer"] [data-testid="stForm"]{
        border:none !important;padding:0 !important;
    }
    [data-testid="stAppViewContainer"] [data-testid="stForm"] > div,
    [data-testid="stAppViewContainer"] [data-testid="stForm"] [data-testid="stVerticalBlock"]{
        gap:6px !important;
    }
    /* Remove all empty spacer blocks in login */
    [data-testid="stAppViewContainer"] [data-testid="stForm"] [data-testid="stVerticalBlock"] > div:empty{
        display:none !important;height:0 !important;margin:0 !important;
    }
    @media(max-width:520px){
        /* Colapsar columnas espaciadoras solo en móvil real */
        [data-testid="stHorizontalBlock"] > div:nth-child(1),
        [data-testid="stHorizontalBlock"] > div:nth-child(3){
            flex:0 0 0px !important;min-width:0 !important;
            overflow:hidden !important;padding:0 !important;
        }
        [data-testid="stHorizontalBlock"] > div:nth-child(2){
            flex:1 1 100% !important;min-width:0 !important;
        }
        .gpanel{padding:18px 16px 16px;border-radius:14px;}
        .orb1,.orb2,.orb3{opacity:0.3;}
    }
    </style>
    ...
    with col:
        ...
        st.markdown(
            ...
            f'box-shadow:0 8px 32px rgba(0,0,0,.18),0 0 0 1px rgba(255,255,255,.1);">'
        ...
        if st.session_state.selected_user_login is None:
            ...
            if st.session_state.get('login_error', False):
                _err_html = "<div style='margin-top:10px;background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.3);border-radius:10px;padding:10px 14px;color:#c0392b;font-size:12px;text-align:center;'>⚠ PIN incorrecto — intenta de nuevo</div>"
            st.markdown(
                f"<div class='gpanel' style='padding-bottom:28px;'>"
                # User row
                "<div style='display:flex;align-items:center;gap:16px;"
                "padding-bottom:18px;margin-bottom:18px;border-bottom:1px solid #e8eef5;'>"
                f"<div style='width:56px;height:56px;border-radius:50%;background:{uc};"
                f"display:flex;align-items:center;justify-content:center;"
                f"font-size:26px;flex-shrink:0;box-shadow:0 4px 18px {uc}55;'>{ua}</div>"
                f"<div><div style='font-weight:700;font-size:19px;color:#0d1b2a;"
                f"font-family:DM Serif Display,serif;'>{un}</div>"
                f"<div style='font-size:11px;color:#6b7c93;margin-top:3px;'>{port_joined}</div>"
                "</div></div>"
                # PIN label
                "<div style='font-size:10px;font-weight:700;color:#c9a227;"
                "letter-spacing:.14em;text-transform:uppercase;margin-bottom:10px;'>PIN de acceso</div>"
                # Error
                f"{_err_html}"
                "</div>",
                unsafe_allow_html=True
            )
            # Inject CSS inputmode before widget renders
            st.markdown("""
            <style>
            /* Force numeric keyboard on all password inputs */
            input[type="password"] { -webkit-text-security: disc; }
            </style>
            <script>
            (function(){
                function setNumeric(){
                    window.parent.document.querySelectorAll('input[type="password"]').forEach(function(el){
                    ...
                    window.parent.document.querySelectorAll("iframe").forEach(function(f){
                        try{
                            f.contentDocument.querySelectorAll('input[type="password"]').forEach(function(el){
                                el.setAttribute("inputmode","numeric");
                                el.setAttribute("pattern","[0-9]*");
                            });
                        }catch(e){}
                ...
                setNumeric();
                setTimeout(setNumeric,200); setTimeout(setNumeric,600); setTimeout(setNumeric,1200);
                new MutationObserver(setNumeric).observe(window.parent.document.body,{childList:true,subtree:true});
            })();
            </script>
            """, unsafe_allow_html=True)
            ...
            with c1:
                back = st.button("← Volver", use_container_width=True, key="back_btn")
            with c2:
...
with st.sidebar:
    # ── EGADE Logo ────────────────────────────────────────────────────────────
    st.markdown(
        "<div style='padding:18px 14px 12px; border-bottom:1px solid rgba(201,162,39,0.2); margin-bottom:16px;'>"
    ...
    # ── User header + TC ──────────────────────────────────────────────────────
    # TC live para mostrar en sidebar
    _tc_sidebar = 20.50
    if YF_AVAILABLE:
        try:
            _tc_live_s = getattr(yf.Ticker("MXN=X").fast_info, "last_price", None)
            if _tc_live_s and 15 < float(_tc_live_s) < 30:
                _tc_sidebar = round(float(_tc_live_s), 2)
        except: pass

    st.markdown(f"""
    <div style='padding:14px 0 0;'>
        <div style='font-family:"DM Serif Display",serif; font-size:20px; color:#e8f0f8; margin-bottom:2px;'>
            Portfolio <span style='color:#c9a227;'>Manager</span>
        </div>
        <div style='font-size:10px; color:#6b7c93; margin-bottom:12px;'>Sistema de Gestión v1.0</div>
    ...
    # TC glassmorphism card — standalone, justo debajo del user card
    st.markdown(f"""
    <div style='
        background:linear-gradient(135deg,rgba(13,27,42,0.95) 0%,rgba(26,58,92,0.9) 100%);
        border:1px solid rgba(201,162,39,0.3);
        border-radius:14px; padding:14px 16px; margin-top:10px;
        box-shadow:0 8px 32px rgba(0,0,0,0.4), 0 0 0 1px rgba(201,162,39,0.1) inset;
        backdrop-filter:blur(12px); position:relative; overflow:hidden;'>
        <div style='position:absolute;top:-20px;right:-20px;width:80px;height:80px;
            border-radius:50%;background:radial-gradient(circle,rgba(201,162,39,0.15) 0%,transparent 70%);'></div>
        ...
        <div style='display:flex;align-items:baseline;gap:6px;'>
            <span style='font-size:28px;font-weight:800;color:#fff;
                font-family:"DM Serif Display",serif;line-height:1;'>{_tc_sidebar:.2f}</span>
            <div style='display:flex;flex-direction:column;gap:1px;'>
                <span style='font-size:9px;color:rgba(255,255,255,0.4);'>tiempo real</span>
    ...
    # ── ECG animation in sidebar — per active portfolio ──────────────────
    _active_port_ecg = st.session_state.get('selected_port', '')
    _has_alerts = st.session_state.get(f'has_alerts_ecg_{_active_port_ecg}',
                  st.session_state.get('has_alerts_ecg', False))
    _ecg_color  = '#ef4444' if _has_alerts else '#1a7a4a'
    _ecg_speed  = '1.4s' if _has_alerts else '2.2s'
    st.markdown(f"""
    <style>
    @keyframes ecg_draw{{
    ...
    .ecg-wrap{{position:relative;height:44px;margin:4px 0 8px;overflow:hidden;}}
    .ecg-wrap svg{{width:100%;height:44px;}}
    .ecg-line{{
        stroke:{_ecg_color};stroke-width:2;fill:none;
        stroke-dasharray:300;stroke-dashoffset:300;
        stroke-linecap:round;stroke-linejoin:round;
        animation:ecg_draw {_ecg_speed} ease-out infinite,
                   ecg_fade {_ecg_speed} ease-out infinite;
        filter:drop-shadow(0 0 3px {_ecg_color});
    }}
    .ecg-dot{{
        fill:{_ecg_color};
        animation:ecg_fade {_ecg_speed} ease-out infinite;
        filter:drop-shadow(0 0 4px {_ecg_color});
    }}
    .ecg-label{{
        font-size:9px;color:rgba(255,255,255,0.4);letter-spacing:.1em;
        text-transform:uppercase;margin-bottom:2px;
    }}
    </style>
    <div class='ecg-label'>{'⚡ Alertas activas' if _has_alerts else '✓ Portafolio estable'}</div>
    <div class='ecg-wrap'>
        <svg viewBox='0 0 200 44' preserveAspectRatio='none'>
            ...
            <path class='ecg-line'
                d='M0,22 L30,22 L38,22 L42,6 L46,38 L50,14 L54,22 L80,22
                   L88,22 L92,6 L96,38 L100,14 L104,22 L130,22
                   L138,22 L142,6 L146,38 L150,14 L154,22 L200,22'/>
            <!-- Traveling dot -->
            <circle class='ecg-dot' cx='154' cy='22' r='2.5'/>
        </svg>
    </div>
    """, unsafe_allow_html=True)
    ...
    sheet_id   = _own_sheet if _own_sheet else _shared_sheet
    use_sheets = bool(sheet_id) and get_gspread_client() is not None
    if not use_sheets and sheet_id:
        st.warning("⚠ Credenciales no configuradas", icon="⚠️")
    st.divider()
    # Portfolio selector
    ...
    # ════════════════════════════════════════════════════════════════════════
    # ANÁLISIS MARKOWITZ — run_complete_analysis() del módulo core_markowitz
    # Solo recalcula cuando el portafolio cambia (cacheado por _mkz_port)
    # ════════════════════════════════════════════════════════════════════════
    _sheets_warn = not use_sheets
    st.session_state["sheets_warn"] = _sheets_warn

    if st.session_state.get("_mkz_port") != selected_port:
        # Portafolio nuevo → recalcular
        _mkz = None; _mkz_ok = False; _prices_df = None
        _ret_source = "MATLAB (estático)"

        if use_sheets and sheet_id:
            _prices_df = load_prices_history_from_sheets(sheet_id, selected_port)
            if _prices_df is not None and len(_prices_df) >= 13:
                try:
                    _mkz       = run_complete_analysis(
                        _prices_df.dropna(axis=1), rf=port_data.get("rf", None)
                    )
                    ...
                    _mkz_ok     = True
                    _ret_source = f"Bloomberg Real · {len(_prices_df)} obs"
                except Exception as _mkz_err:
                    target_wts  = port_data["target"]
                    _ret_source = f"Error análisis: {str(_mkz_err)[:60]}"
            else:
                target_wts  = port_data["target"]
                _ret_source = "Datos insuficientes en Sheet"
        else:
            target_wts = port_data["target"]

        # Persistir resultados en session_state
        st.session_state.update({
            "mkz": _mkz, "mkz_ok": _mkz_ok, "ret_source": _ret_source,
            "target_wts": target_wts, "prices_df": _prices_df,
            "_mkz_port": selected_port,
        })
    else:
        # Mismo portafolio → reutilizar session_state
        target_wts = st.session_state.get("target_wts") or port_data["target"]
    # ECG: will be updated after alerts_df is computed below



    
    # TC ya obtenido arriba (_tc_sidebar) — reusar
    _tc_yf = _tc_sidebar
    _is_admin_sb = user_data.get("role") == "admin"
    if _is_admin_sb:
        st.markdown("<div style='font-size:11px;font-weight:700;letter-spacing:.1em;color:#c9a227;text-transform:uppercase;margin-bottom:8px;'>Parámetros</div>", unsafe_allow_html=True)
        tc_actual     = st.number_input("TC MXN/USD", value=_tc_yf, step=0.01, format="%.2f",
            help=f"Yahoo Finance: {_tc_yf:.2f} · editable para ops de otra fecha")
...
# ── MOBILE NAVIGATION (pills, visible only on small screens via CSS) ──────────
...
# Update page if user selected from mobile nav
if _mob_sel and _mob_sel != page:
    page = _mob_sel
...
if use_sheets and sheet_id:
    ...
    if ops_raw is not None and not ops_raw.empty:
        if "Usuario" in ops_raw.columns:
            _u_name_full = user_data.get("name", current_user).lower()
            _u_col = ops_raw["Usuario"].astype(str).str.strip().str.lower()
            _mask  = (
                (_u_col == current_user.lower()) |
                (_u_col == _u_name_full) |
                (_u_col == _u_name_full.split()[0])
            )
            _f = ops_raw[_mask].copy()
            ops_df = _f if not _f.empty else ops_raw
        else:
            ops_df = ops_raw
...
# Precios para valor de cartera: ops_prices tiene PRIORIDAD
# El Sheet Bloomberg es para el modelo (retornos históricos), no para valorar posiciones
_ops_prices = {}
if not ops_df.empty and "Precio_USD" in ops_df.columns:
    _ops_prices = (
        ops_df.sort_values("Fecha")
        .groupby("Ticker")["Precio_USD"].last().to_dict()
    )

if _ops_prices:
    # ops_prices primero — precio real de cada ticker en este portafolio
    # Sheet Bloomberg rellena tickers que no están en ops (p.ej. tickers del modelo sin compras aún)
    _sheet_prices = prices or {}
    prices = {**_sheet_prices, **_ops_prices}  # ops sobreescribe Sheet
    prices_source = "Precio registrado en operaciones"
elif prices is None:
    prices = get_demo_prices()
    prices_source = "Demo"
...
_saludo = 'Buenos días' if 6 <= _hora < 13 else 'Buenas tardes' if 13 <= _hora < 20 else 'Buenas noches'
_nombre_display = user_data.get('name', current_user)
st.markdown(f"""
<div class="app-banner">
    <div class="banner-left">
        <div class="banner-title">{selected_port}</div>
        <div class="banner-sub">{port_data['description']}</div>
        <div style="font-size:11px; color:rgba(201,162,39,0.8); margin-top:4px;">Precios: {prices_source}</div>
        <div class="banner-date">Actualizado: {datetime.now().strftime('%d %b %Y, %H:%M')}</div>
    </div>
    <div class="banner-right">
        <div class="banner-count"><span class="num-anim">{len(target_wts)}</span></div>
        <div class="banner-count-label">activos en portafolio</div>
        <div class="banner-greeting">{_saludo}, <b>{_nombre_display}</b></div>
    </div>
</div>
""", unsafe_allow_html=True)
...
# _active_tickers: modelo + ops del portafolio activo (ops_df ya está filtrado por portafolio)
_model_tickers  = list(target_wts.keys())
_ops_tickers    = ops_df["Ticker"].unique().tolist() if not ops_df.empty else []
_active_tickers = _model_tickers + [t for t in _ops_tickers if t not in _model_tickers]
...
_has_actions_ecg = not alerts_df.empty and (alerts_df["Alerta"].isin(["COMPRAR","VENDER"])).any()
# Store per-portfolio ECG state
st.session_state[f'has_alerts_ecg_{selected_port}'] = _has_actions_ecg
st.session_state['has_alerts_ecg'] = _has_actions_ecg
st.session_state['ecg_port'] = selected_port
# Auto-email: only fires once per session when new alerts appear
_email_key = f"emailed_{current_user}_{selected_port}"
if _has_actions_ecg and not st.session_state.get(_email_key, False):
    _umail = user_data.get("email", "")
    if _umail:
        _ar = alerts_df[alerts_df["Alerta"].isin(["COMPRAR","VENDER"])].to_dict("records")
        _nb = sum(1 for r in _ar if r.get("Alerta") == "COMPRAR")
        _ns = sum(1 for r in _ar if r.get("Alerta") == "VENDER")
        send_alert_email(user_data.get("name", current_user), _umail,
                         selected_port, _nb, _ns, _ar)
st.session_state[_email_key] = _has_actions_ecg
_bbg_rm = {}
# Derivar retornos logarítmicos desde _prices_df (cargado en el sidebar)
# _prices_df se guarda en session_state por el bloque de Markowitz del sidebar
_prices_df_rm = st.session_state.get("prices_df")
if _prices_df_rm is not None and target_wts:
    try:
        _log_ret_rm = np.log(_prices_df_rm / _prices_df_rm.shift(1)).dropna()
        _bbg_rm     = calc_risk_metrics_bloomberg(
            _log_ret_rm, target_wts, rf=port_data.get("rf", 0.0264)
        )
    except Exception:
        _bbg_rm = {}
...
# ══════════════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════════
# BANNER GLOBAL: MODO DEMO (sin Google Sheets)
# Se muestra en todas las páginas cuando use_sheets=False
# ═══════════════════════════════════════════════════════════════════════════
if st.session_state.get("sheets_warn", False):
    st.warning(
        "⚠️ **Modo Demo** — Sin conexión a Google Sheets. "
        "Los datos de precios, operaciones y pesos mostrados son **ilustrativos**. "
        "Para datos reales, configura las credenciales en `.streamlit/secrets.toml`.",
        icon="⚠️"
    )
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    ...
    if not _is_admin:
        ...
        if has_actions:
            ...
            status_bg     = "linear-gradient(135deg,#f9d100 0%,#fce033 100%)"
            status_border = "#e6c200"
            status_txt    = "#1a1400"
            status_sub    = "#5c4d00"
            _pulse_rgb    = "249,209,0"
        else:
            ...
            status_txt    = "#1a2332"
            status_sub    = "#6b7c93"
            _pulse_rgb    = "26,122,74"

        _pulse_anim = f"palert_{'warn' if has_actions else 'ok'}"
        st.markdown(f"""
        <style>
        @keyframes {_pulse_anim}{{
            0%,100%{{box-shadow:0 0 0 0 rgba({_pulse_rgb},0.55),0 2px 16px rgba(0,0,0,0.08);}}
            55%{{box-shadow:0 0 0 10px rgba({_pulse_rgb},0),0 2px 16px rgba(0,0,0,0.08);}}
        }}
        ...
        <div style='background:{status_bg};border-left:6px solid {status_border};
                    border-radius:14px;padding:16px 20px 14px;margin-bottom:20px;
                    animation:{_pulse_anim} {'2s ease-in-out infinite' if has_actions else 'none'};
                    position:relative;'>
            <div style='display:flex;align-items:center;gap:10px;margin-bottom:5px;'>
                <span style='display:inline-block;width:11px;height:11px;border-radius:50%;
                    background:{status_border};
                    animation:{'blink_alert 1s ease-in-out infinite' if has_actions else 'none'};
                    flex-shrink:0;box-shadow:0 0 7px {status_border};'></span>
                <div style='font-size:17px;font-family:"DM Serif Display",serif;
                    color:{status_txt};line-height:1.3;font-weight:700;'>{status_msg}</div>
            </div>
            <div style='font-size:11px;color:{status_sub};padding-left:21px;'>
        ...
        _h = (
            ...
            f"<div style='font-size:26px;font-weight:800;color:{pc};letter-spacing:-0.01em;line-height:1.1;'>{rlo}% – {rhi}%</div>"
            f"<div style='font-size:10px;color:#6b7c93;margin-top:2px;'>Sharpe objetivo: &gt; {sobj}</div>"
            f"</div>"
            f"</div>"  # end grid
            f"</div>"  # end card
        )
        ...
        with k1:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-label">Valor en este portafolio</div>
            ...
            if live_q_inv:
                ...
                for i, tkr in enumerate(target_wts.keys()):
                    ...
                    with val_cols[i]:
                        ...
                        bar_pct = max(0,min(100,(price-w52_low)/(w52_high-w52_low)*100)) if (w52_high and w52_low and w52_high>w52_low) else 50
                        st.markdown(
                            ...
                            f"<div style='font-size:10px;color:#6b7c93;margin-bottom:4px;'>Rango 52 semanas · Prom: <b style='color:#1a3a5c;'>{ma_str}</b></div>"
                            ...
                            f"<div style='position:absolute;left:{bar_pct:.0f}%;top:-4px;width:14px;height:14px;"
                            f"border-radius:50%;background:#1a3a5c;transform:translateX(-50%);"
                            f"border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.2);'></div></div>"
        ...
        # ── Mercado en Tiempo Real (carousel) ─────────────────────────────────
        st.markdown("<div class='section-title'>📡 Mercado en Tiempo Real</div>", unsafe_allow_html=True)
        st.markdown(
            "<div style='font-size:12px;color:#6b7c93;margin-bottom:12px;'>"
            "Movimiento del día de cada acción en tu portafolio. "
            "Las variaciones diarias son normales — lo que importa es la tendencia a largo plazo.</div>",
            unsafe_allow_html=True)
        ...
        if not YF_AVAILABLE:
            ...
            with st.spinner("Cargando datos de mercado..."):
                quotes_inv = get_live_quotes(list(target_wts.keys()))
            if not quotes_inv:
                st.info("No se pudieron cargar cotizaciones. Verifica tu conexión.")
            else:
                cards_html_inv = ""
                for tkr in target_wts.keys():
                    q        = quotes_inv.get(tkr, {})
                    price    = q.get("price")
                    open_p   = q.get("open")
                    ...
                    chg_icon  = "▲" if (chg_pct or 0) >= 0 else "▼"
                    chg_str   = f"{chg_icon} {abs(chg_pct):.2f}%" if chg_pct is not None else "—"
                    range_pct = max(0,min(100,(price-day_low)/(day_high-day_low)*100)) if (day_high and day_low and day_high>day_low) else 50
                    _op   = f"${open_p:,.2f}"   if open_p   else "—"
                    _pc   = f"${prev_c:,.2f}"   if prev_c   else "—"
                    _dh   = f"${day_high:,.2f}" if day_high else "—"
                    _dl   = f"${day_low:,.2f}"  if day_low  else "—"
                    cards_html_inv += (
                        "<div class='mkt-card'>"
                        f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;'>"
                        f"<span style='font-weight:700;font-size:15px;color:#1a2332;'>{tkr}</span>"
                        ...
                        f"<div>Apertura <b style='color:#1a2332;'>{_op}</b></div>"
                        f"<div>Cierre ant. <b style='color:#1a2332;'>{_pc}</b></div>"
                        f"<div>Max <b style='color:#1a7a4a;'>{_dh}</b></div>"
                        f"<div>Min <b style='color:#c0392b;'>{_dl}</b></div></div>"
                        ...
                        f"<div style='position:absolute;left:{range_pct:.0f}%;top:-3px;width:14px;height:14px;"
                        "border-radius:50%;background:#1a3a5c;transform:translateX(-50%);"
                        "border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.2);'></div></div>"
                        f"<div style='display:flex;justify-content:space-between;font-size:9px;color:#aab;margin-top:3px;'>"
                        f"<span>{_dl}</span><span>{_dh}</span></div>"
                        "</div>")
                n_c = len(target_wts)
                carousel_inv = f"""
    ...
    <style>
      .mkt-carousel-wrap {{ overflow:hidden; width:100%; position:relative; }}
      ...
      .mkt-track {{ display:flex; gap:16px; width:max-content;
                    animation:mkt-scroll {n_c*5}s linear infinite; }}
      .mkt-track:hover {{ animation-play-state:paused; }}
      @keyframes mkt-scroll {{ 0% {{ transform:translateX(0); }} 100% {{ transform:translateX(-50%); }} }}
      .mkt-card {{ background:white; border-radius:14px; padding:16px 14px;
                   border:1px solid #e8eef5; box-shadow:0 2px 10px rgba(13,27,42,0.07);
                   min-width:230px; max-width:230px; flex-shrink:0; }}
      .mkt-card:hover {{ box-shadow:0 6px 20px rgba(13,27,42,0.14); transform:translateY(-3px); }}
    </style>
    <div class="mkt-carousel-wrap"><div class="mkt-track">
      {cards_html_inv}{cards_html_inv}
    </div></div>"""
                st.markdown(carousel_inv, unsafe_allow_html=True)

        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)

        # ── Donut chart: actual vs target ─────────────────────────────────────
        if not alerts_df.empty:
            st.markdown("<div class='section-title'>📈 Distribución de tu portafolio</div>",
                        unsafe_allow_html=True)
            st.markdown(
                "<div style='font-size:12px;color:#6b7c93;margin-bottom:10px;'>"
                "La dona de la izquierda es cómo está hoy tu portafolio. "
                "La de la derecha es cómo debería estar. "
                "Entre más parecidas, mejor.</div>",
                unsafe_allow_html=True)

            colors_p = ["#1a3a5c","#c9a227","#1a7a4a","#c0392b","#2d4a6b","#d4820a"]
            fig_pie  = make_subplots(
                rows=1, cols=2, specs=[[{"type":"pie"},{"type":"pie"}]],
                subplot_titles=["Cómo está hoy", "Cómo debería estar"]
            )
            fig_pie.add_trace(go.Pie(
                labels=alerts_df["Ticker"], values=alerts_df["Peso Actual"],
                hole=0.5, marker_colors=colors_p,
                textinfo="label+percent", textfont_size=12, showlegend=False,
                hovertemplate="<b>%{label}</b><br>Hoy: %{percent}<extra></extra>"
            ), row=1, col=1)
            fig_pie.add_trace(go.Pie(
        ...
        # ── Portfolio evolution ────────────────────────────────────────────────
        st.markdown("<div class='section-title'>📈 Evolución de mi inversión</div>",
                    unsafe_allow_html=True)
        st.markdown(
            "<div style='font-size:12px;color:#6b7c93;margin-bottom:10px;'>"
            "Valor total de tu portafolio mes a mes en pesos mexicanos.</div>",
            unsafe_allow_html=True)
        ...
        if not history.empty:
            ...
            fig_hist.add_trace(go.Bar(
                ...
                marker_color=["#1a7a4a" if r >= 0 else "#c0392b" for r in rets],
                opacity=0.65, yaxis="y2", name="Retorno mensual",
                hovertemplate="<b>%{x}</b><br>%{y:.1f}%<extra></extra>"
            ))
            fig_hist.update_layout(
                height=300, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.25)",
                font=dict(family="DM Sans", color="#1a2332"),
        ...
        with st.expander("➕ Registrar una operación ahora", expanded=False):
            with st.form("inv_quick_op", clear_on_submit=True):
                qa, qb, qc = st.columns(3)
                with qa:
                    q_fecha  = st.date_input("Fecha", value=date.today())
                    q_tipo   = st.selectbox("Tipo", ["Compra", "Venta"])
                with qb:
                    q_ticker = st.text_input("Ticker", placeholder="NVDA").upper()
                    q_cant   = st.number_input("Cantidad", min_value=0.0001, value=1.0, step=0.0001, format="%.4f")
                with qc:
                    q_precio = st.number_input("Precio USD", min_value=0.01, value=100.0, step=0.01)
                    q_com    = st.number_input("Comisión USD", min_value=0.0, value=0.50, step=0.01)
                q_tc = st.number_input("TC MXN/USD", value=tc_actual, step=0.01, format="%.2f")
                if st.form_submit_button("💾 Registrar", use_container_width=True):
                    if q_ticker:
                        if use_sheets:
                            ok = save_row(sheet_id, f"Ops_{selected_port}",
                                          [str(q_fecha), q_ticker, q_tipo, q_cant, q_precio, q_com, q_tc, current_user])
                            if ok:
                                st.success(f"✅ {q_tipo} de {q_cant:.4f} {q_ticker} registrada")
                                st.cache_data.clear()
                            else:
                                st.error("Error al guardar en Sheets")
                        else:
                            st.success(f"✅ {q_tipo} de {q_cant:.4f} {q_ticker} @ ${q_precio:.2f} (demo)")
                    else:
                        st.error("Ingresa un ticker")

    # ══════════════════════════════════════════════════════════════════════════
    # ADMIN VIEW — full dashboard (existing logic)
    # ══════════════════════════════════════════════════════════════════════════
    else:
        ...
        INVESTOR_PROFILES = {
            "Tangencia": {
                ...
                "border":    "#c9a227",
                "objetivo":  "Maximizar el Sharpe Ratio — mejor retorno por unidad de riesgo",
                "horizonte": "Mediano–largo plazo · 3–7 años",
                "tolerancia":"Media · acepta volatilidad moderada a cambio de crecimiento",
                "perfil":    "Inversionista disciplinado que busca eficiencia. Diversifica entre crecimiento y valor. Rebalancea activamente cuando hay desviaciones.",
                "stats": [
                    ("Volatilidad esperada", "~20–28%"),
                    ("Retorno objetivo",     "~30–35%"),
                    ("Sharpe objetivo",      "> 1.2"),
            ...
            "Conservador": {
                ...
                "border":    "#1a7a4a",
                "objetivo":  "Minimizar la varianza — preservar capital ante todo",
                "horizonte": "Corto–mediano plazo · 1–4 años",
                "tolerancia":"Baja · prioriza estabilidad sobre rendimiento",
                "perfil":    "Inversionista adverso al riesgo. Prefiere activos defensivos (oro, tabaco, industria estable). Acepta retornos más bajos a cambio de protección en caídas.",
                "stats": [
                    ("Volatilidad esperada", "~8–14%"),
                    ("Retorno objetivo",     "~10–16%"),
                    ("Sharpe objetivo",      "> 0.8"),
                    ("Horizonte mínimo",     "1 año"),
                ],
                "teoria": "Markowitz: portafolio de Mínima Varianza — punto más a la izquierda de la Frontera Eficiente.",
            },
            "Agresivo": {
                ...
                "tolerancia":"Alta · tolera drawdowns fuertes y volatilidad elevada",
                "perfil":    "Inversionista con alta convicción en tendencias disruptivas (IA, EVs, tecnología). Concentra posiciones en activos de alto beta. Requiere horizonte largo para absorber ciclos.",
                "stats": [
                    ("Volatilidad esperada", "~35–55%"),
                    ("Retorno objetivo",     "~50–80%"),
                    ("Sharpe objetivo",      "> 1.0"),
                    ("Horizonte mínimo",     "5 años"),
                ],
                "teoria": "Portafolio en la parte superior derecha de la Frontera Eficiente — máximo retorno esperado con alta concentración.",
            },
            "Familiar": {
                "label":     "PATRIMONIAL",
                "icon":      "🏛️",
                "color":     "#2d4a6b",
                "bg":        "linear-gradient(135deg,#eef3f8 0%,#d6e4f0 100%)",
                "border":    "#2d4a6b",
                "objetivo":  "Preservar y transmitir patrimonio intergeneracional",
                "horizonte": "Muy largo plazo · 10+ años",
                "tolerancia":"Muy baja · la protección del capital es prioritaria",
                "perfil":    "Gestión de riqueza familiar. Combina activos refugio (oro, tabaco) con activos de valor estable (banca, industria). Minimiza riesgo sistémico y concentración en un solo sector.",
                "stats": [
                    ("Volatilidad esperada", "~10–16%"),
                    ("Retorno objetivo",     "~12–18%"),
                    ("Sharpe objetivo",      "> 0.7"),
                    ("Horizonte mínimo",     "10 años"),
                ],
                "teoria": "Estrategia mixta: activos de baja correlación entre sí para maximizar diversificación (Riesgo No-Sistemático → 0).",
            },
        }

        prof = INVESTOR_PROFILES.get(selected_port, INVESTOR_PROFILES["Tangencia"])
        p_color  = prof["color"]
        p_border = prof["border"]

        col_prof, col_stats = st.columns([1.7, 1])

        with col_prof:
            stats_html = "".join([
                f"<div style='display:flex; justify-content:space-between; padding:5px 0;"
                f"border-bottom:1px solid rgba(0,0,0,0.06);'>"
                f"<span style='color:#6b7c93; font-size:11px;'>{k}</span>"
                f"<span style='font-weight:700; font-size:11px; color:{p_color};'>{v}</span>"
                f"</div>"
                for k, v in prof["stats"]
            ])
            st.markdown(f"""
            <div class="anim-fadeinup" style='background:{prof["bg"]}; border-radius:16px;
                        padding:22px 26px; border-left:5px solid {p_border};
                        box-shadow:0 2px 16px rgba(0,0,0,0.07); margin-bottom:4px;'>
                <div style='display:flex; align-items:center; gap:14px; margin-bottom:14px;'>
                    <div style='font-size:36px; line-height:1;'>{prof["icon"]}</div>
                    <div>
                        <div style='font-size:11px; letter-spacing:0.12em; font-weight:700;
                                    color:{p_color}; text-transform:uppercase;'>Perfil del Inversionista</div>
                        <div style='font-family:"DM Serif Display",serif; font-size:26px;
                                    color:#1a2332; line-height:1.1;'>{prof["label"]}</div>
                    </div>
                </div>
                <div style='font-size:12px; color:#333; line-height:1.65;
                            margin-bottom:12px;'>{prof["perfil"]}</div>
                <div style='display:grid; grid-template-columns:1fr 1fr; gap:10px;
                            font-size:11px;'>
                    <div>
                        <span style='color:#6b7c93; font-weight:600;'>🎯 Objetivo</span><br>
                        <span style='color:#1a2332;'>{prof["objetivo"]}</span>
                    </div>
                    <div>
                        <span style='color:#6b7c93; font-weight:600;'>⏱ Horizonte</span><br>
                        <span style='color:#1a2332;'>{prof["horizonte"]}</span>
                    </div>
                    <div>
                        <span style='color:#6b7c93; font-weight:600;'>💓 Tolerancia al riesgo</span><br>
                        <span style='color:#1a2332;'>{prof["tolerancia"]}</span>
                    </div>
                    <div>
                        <span style='color:#6b7c93; font-weight:600;'>📐 Base teórica</span><br>
                        <span style='color:#1a2332;'>{prof["teoria"]}</span>
                    </div>
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        with col_stats:
            st.markdown(f"""
            <div class="anim-fadeinup anim-delay-1" style='background:white; border-radius:16px;
                ...
                <div style='font-size:11px; font-weight:700; letter-spacing:0.1em;
                            ...
                            margin-bottom:14px;'>Métricas Objetivo</div>
                {stats_html}
                <div style='margin-top:14px; padding:10px 12px; border-radius:10px;
                            background:{prof["bg"]}; text-align:center;'>
                    <div style='font-size:10px; color:#6b7c93; margin-bottom:4px;'>Clasificación de riesgo</div>
                    <div style='font-family:"DM Serif Display",serif; font-size:20px; color:{p_color};'>
        ...
        total_ganancia = alerts_df["Ganancia USD"].sum() if not alerts_df.empty else 0
        gan_pct = (total_ganancia / (total_usd - total_ganancia)) if (total_usd - total_ganancia) > 0 else 0
        sharpe_val = risk_metrics.get("sharpe", 0)
    
        with c1:
            st.markdown(f"""
            <div class="kpi-card anim-fadeinup anim-delay-1">
                <div class="kpi-label">Valor en este Portafolio</div>
                <div class="kpi-value">${total_mxn:,.0f}</div>
                <div class="kpi-sub">MXN · TC {tc_actual:.2f} · ${total_usd:,.0f} USD</div>
            </div>""", unsafe_allow_html=True)
    
        with c2:
            sign_class = "positive" if total_ganancia >= 0 else "negative"
            sign = "+" if total_ganancia >= 0 else ""
            st.markdown(f"""
            <div class="kpi-card anim-fadeinup anim-delay-2">
                <div class="kpi-label">Ganancia / Pérdida</div>
                <div class="kpi-value {sign_class}">{sign}${total_ganancia:,.0f}</div>
                <div class="kpi-sub">USD · {sign}{gan_pct*100:.1f}% desde costo</div>
            </div>""", unsafe_allow_html=True)
    
        with c3:
            sr_label, sr_color = sharpe_label(sharpe_val)
            st.markdown(f"""
            <div class="kpi-card anim-fadeinup anim-delay-3">
                <div class="kpi-label">Sharpe Ratio</div>
                <div class="kpi-value" style="color:{sr_color};">{sharpe_val:.3f}</div>
                <div class="kpi-sub">{sr_label}</div>
            </div>""", unsafe_allow_html=True)
    
        with c4:
            vol = risk_metrics.get("vol_annual", 0)
            st.markdown(f"""
            <div class="kpi-card anim-fadeinup anim-delay-4">
                <div class="kpi-label">Volatilidad Anualizada</div>
                <div class="kpi-value">{vol*100:.1f}%</div>
                <div class="kpi-sub">Últimos {risk_metrics.get('n_months',0)} meses</div>
            </div>""", unsafe_allow_html=True)
        ...
        with col_right:
            ...
            if not alerts_df.empty:
                ...
                _palette = ["#1a3a5c","#c9a227","#1a7a4a","#c0392b","#5b4fcf","#0e7490","#b45309","#6d28d9"]
                _all_tickers = list(dict.fromkeys(
                    list(alerts_df["Ticker"]) + list(target_wts.keys())
                ))
                _tcolor = {t: _palette[i % len(_palette)] for i, t in enumerate(_all_tickers)}
                colors_act = [_tcolor[t] for t in alerts_df["Ticker"]]
                tickers_obj = list(target_wts.keys())
                weights_obj = list(target_wts.values())
                colors_obj  = [_tcolor[t] for t in tickers_obj]
                ...
                fig_pie.add_trace(go.Pie(
                    ...
                    hole=0.52,
                    marker=dict(colors=colors_act, line=dict(color="rgba(255,255,255,0.7)", width=2)),
                    textinfo="label+percent", textfont_size=11,
                    showlegend=True, legendgroup="tickers",
                    hovertemplate=(
                        "<b>%{label}</b><br>"
                ...
                fig_pie.add_trace(go.Pie(
                    ...
                    values=weights_obj,
                    hole=0.52,
                    marker=dict(colors=colors_obj, line=dict(color="rgba(255,255,255,0.7)", width=2)),
                    textinfo="label+percent", textfont_size=11,
                    showlegend=False, legendgroup="tickers",
                    hovertemplate="<b>%{label}</b><br>Peso objetivo: %{percent}<extra></extra>"
                ), row=1, col=2)

                fig_pie.update_layout(
                    height=340, margin=dict(l=20, r=20, t=55, b=55),
                    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                    font=dict(family="DM Sans", color="#1a2332"),
                    legend=dict(orientation="h", yanchor="bottom", y=-0.28,
                                xanchor="center", x=0.5, font_size=11),
                    annotations=[
                        dict(text="Actual", x=0.18, y=0.5, font_size=13,
                             font_color="#1a3a5c", showarrow=False, font_family="DM Serif Display"),
                        dict(text="Objetivo", x=0.82, y=0.5, font_size=13,
                             font_color="#1a3a5c", showarrow=False, font_family="DM Serif Display")
                    ]
                )
        ...
        if not history.empty:
            ...
            fig_hist.update_layout(
                height=320, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.25)",
                font=dict(family="DM Sans", color="#1a2332"),
                margin=dict(l=0, r=0, t=10, b=0),
                legend=dict(orientation="h", yanchor="bottom", y=1.02, font_size=11),
    ...
    with st.form("new_operation", clear_on_submit=True):
        ...
        if submitted:
            if ticker:
                ...
                if use_sheets:
                    row_list = [str(fecha), ticker, tipo, cantidad, precio, comision, tc_op, current_user]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not history.empty and len(history) > 3:
        ...
        with col_dist:
            ...
            fig_dist.update_layout(
                height=280, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.25)",
    ...
    if not history.empty:
        ...
        fig_dd.update_layout(
            height=260, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.25)",
    ...
    with st.form("inv_op_form", clear_on_submit=True):
        ...
        with c2:
            # Universo completo del portafolio activo
            _universe = port_data.get("universe", list(target_wts.keys()))
            _op_existing = ops_df["Ticker"].unique().tolist() if not ops_df.empty else []
            port_tickers_list = _universe + [t for t in _op_existing if t not in _universe]
        ...
        if submitted:
            if use_sheets:
                ok = save_row(sheet_id, f"Ops_{selected_port}",
                              [str(i_fecha), i_ticker, i_tipo, i_cant, i_precio, i_com, i_tc, current_user])
                if ok:
    ...
    all_ports_risk = list(get_demo_portfolios().keys())
    if "risk_selected_port" not in st.session_state:
        st.session_state.risk_selected_port = all_ports_risk[0]
    _rh1, _rh2 = st.columns([3, 1])
    with _rh1:
        st.markdown('<div class="section-title">⚡ Análisis de Riesgo</div>', unsafe_allow_html=True)
    with _rh2:
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        _rsel = st.selectbox("Portafolio", all_ports_risk,
            index=all_ports_risk.index(st.session_state.risk_selected_port)
            if st.session_state.risk_selected_port in all_ports_risk else 0,
            key="risk_port_sel", label_visibility="collapsed")
        if _rsel != st.session_state.risk_selected_port:
            st.session_state.risk_selected_port = _rsel
            st.rerun()
    _rp       = st.session_state.risk_selected_port
    _rp_data  = get_demo_portfolios().get(_rp, {})
    _rp_target = _rp_data.get("target", {})

    # ── Determinar si el portafolio seleccionado en riesgo coincide con el activo ──
    # Si coincide, reusar los resultados ya calculados en el sidebar (sin re-calcular).
    # Si difiere (admin cambió el selector), calcular en el momento.
    if _rp == selected_port and st.session_state.get("mkz_ok", False):
        # Mismo portafolio que la barra lateral: reusar
        _rp_mkz     = st.session_state.get("mkz")
        _rp_mkz_ok  = True
        _rp_prices  = st.session_state.get("prices_df")
        _rp_source  = st.session_state.get("ret_source", "Bloomberg Real")
    else:
        # Portafolio diferente: cargar precios y calcular desde cero
        _rp_mkz    = None
        _rp_mkz_ok = False
        _rp_prices = None
        _rp_source = "MATLAB (estático)"
        if use_sheets and sheet_id:
            _rp_prices = load_prices_history_from_sheets(sheet_id, _rp)
            if _rp_prices is not None and len(_rp_prices) >= 13:
                try:
                    _rp_mkz    = run_complete_analysis(
                        _rp_prices.dropna(axis=1),
                        rf=_rp_data.get("rf", None)
                    )
                    _rp_mkz_ok  = True
                    _rp_source  = f"Bloomberg Real · {len(_rp_prices)} obs"
                    # Actualizar pesos del portafolio seleccionado en riesgo
                    _rp_target = {t: w for t, w in _rp_mkz["tangencia"].items() if w >= 0.001}
                    if not _rp_target:
                        _rp_target = _rp_data.get("target", {})
                except Exception as _re:
                    _rp_source = f"Error: {str(_re)[:60]}"

    # ── Calcular métricas de riesgo usando log-retornos del prices_df ──────────
    # Si tenemos precios reales, calculamos retornos logarítmicos y luego risk metrics
    _rp_ret_raw = None
    if _rp_prices is not None and len(_rp_prices) >= 13:
        try:
            _prices_clean = _rp_prices.dropna(axis=1)
            _log_rets     = np.log(_prices_clean / _prices_clean.shift(1)).dropna()
            _rp_ret_raw   = _log_rets   # DataFrame de retornos logarítmicos
        except Exception:
            _rp_ret_raw = None

    if _rp_ret_raw is not None and _rp_target:
        risk_metrics = calc_risk_metrics_bloomberg(
            _rp_ret_raw, _rp_target, rf=_rp_data.get("rf", 0.0264)
        )
    else:
        _rp_ops  = get_demo_operations(_rp)
        _rp_hist = calc_portfolio_history(_rp_ops, {}, 1.0, _rp_target)
        risk_metrics = calc_risk_metrics(_rp_hist)

    # Badge de fuente de datos
    _src_col = "#1a7a4a" if "Bloomberg" in _rp_source else "#d4820a"
    st.markdown(
        f"<div style='display:inline-block;background:{_src_col}22;border:1px solid {_src_col};"
        f"border-radius:20px;padding:3px 14px;font-size:11px;color:{_src_col};"
        f"font-weight:700;margin-bottom:10px;'>📡 Fuente: {_rp_source}</div>",
        unsafe_allow_html=True
    )
    if not _rp_mkz_ok:
        st.warning(
            "ℹ️ Sin precios históricos de Google Sheets, el análisis Markowitz "
            "completo (frontera eficiente, correlación real, Monte Carlo) no está disponible. "
            "Conecta la hoja **Precios_" + _rp + "** para activarlo.",
            icon="📊"
        )
    # ── Downloads
    _dl1, _dl2, _dlsp = st.columns([1, 1, 4])
    with _dl1:
        try:
            _rp_ops2  = get_demo_operations(_rp)
            _rp_pos2  = calc_positions(_rp_ops2, {}, tc_actual, list(_rp_target.keys()))
            _rp_hist2 = calc_portfolio_history(_rp_ops2, {}, tc_actual, _rp_target)
            _rp_alr2  = calc_alerts(_rp_pos2, _rp_target, tc_actual, 5000, 0.05, 0.10)
            _xl2 = build_portfolio_excel(_rp, _rp_data, _rp_pos2, _rp_hist2, _rp_alr2, risk_metrics, _rp_target, tc_actual)
            st.download_button("⬇ Excel", data=_xl2,
                file_name="Riesgo_"+_rp+"_"+datetime.now().strftime("%Y%m%d")+".xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True, key="risk_xl_"+_rp)
        except Exception: st.caption("Excel N/D")
    with _dl2:
        try:
            _rp_ops2  = get_demo_operations(_rp)
            _rp_pos2  = calc_positions(_rp_ops2, {}, tc_actual, list(_rp_target.keys()))
            _rp_alr2  = calc_alerts(_rp_pos2, _rp_target, tc_actual, 5000, 0.05, 0.10)
            _sh2=risk_metrics.get("sharpe",0);_re2=risk_metrics.get("ret_annual",0)*100
            _vo2=risk_metrics.get("vol_annual",0)*100;_va2=abs(risk_metrics.get("var_95",0))*100
            _pt2="".join(f"<tr><td><b>{r.get('Ticker','')}</b></td><td>{r.get('Acciones',0):.1f}</td><td>${r.get('Precio USD',0):,.2f}</td><td>${r.get('Valor MXN',0):,.0f}</td></tr>" for _,r in _rp_pos2.iterrows()) if not _rp_pos2.empty else ""
            _at2="".join(f"<tr style='background:#fff9c4'><td><b>{r.get('Ticker','')}</b></td><td style='color:red;font-weight:700'>{r.get('Alerta','')}</td><td>{r.get('Actual %','')}</td><td>{r.get('Objetivo %','')}</td></tr>" for _,r in _rp_alr2[_rp_alr2["Alerta"].isin(["COMPRAR","VENDER"])].iterrows()) if not _rp_alr2.empty else ""
            _wt2="".join(f"<tr><td><b>{t}</b></td><td>{w*100:.1f}%</td></tr>" for t,w in _rp_target.items())
            _html2 = ("<!DOCTYPE html><html><head><meta charset='utf-8'><style>body{font-family:Arial;background:#edf2f8;margin:0}.page{max-width:860px;margin:24px auto;background:white;border-radius:14px;overflow:hidden}.hdr{background:linear-gradient(135deg,#0d2240,#1a3a5c);padding:26px 34px}.hdr h1{color:#c9a227;font-size:22px;margin:0}.body{padding:26px 34px}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:16px 0}.kpi{background:#edf2f8;border-radius:10px;padding:12px;text-align:center}.kpi .v{font-size:20px;font-weight:800;color:#1a3a5c}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#1a3a5c;color:#fff;padding:9px 12px}td{padding:7px 12px;border-bottom:1px solid #f0f4f9}</style></head><body><div class='page'>"
                "<div class='hdr'><h1>Riesgo "+_rp+"</h1><p style='color:#9ab0c8;font-size:11px;'>"+datetime.now().strftime('%d %b %Y')+" · EGADE</p></div>"
                "<div class='body'><div class='kpis'>"
                "<div class='kpi'><div class='v'>"+str(round(_sh2,2))+"</div><div>Sharpe</div></div>"
                "<div class='kpi'><div class='v'>"+str(round(_re2,1))+"%</div><div>Retorno</div></div>"
                "<div class='kpi'><div class='v'>"+str(round(_vo2,1))+"%</div><div>Volatilidad</div></div>"
                "<div class='kpi'><div class='v'>"+str(round(_va2,1))+"%</div><div>VaR</div></div>"
                "</div><h3>Posiciones</h3><table><tr><th>Ticker</th><th>Acc</th><th>Precio</th><th>Valor MXN</th></tr>"+_pt2+"</table>"
                +("<h3>Alertas</h3><table><tr><th>Activo</th><th>Accion</th><th>Actual</th><th>Obj</th></tr>"+_at2+"</table>" if _at2 else "")
                +"<h3>Pesos Objetivo</h3><table><tr><th>Ticker</th><th>Peso</th></tr>"+_wt2+"</table>"
                +"</div></div></body></html>")
            st.download_button("⬇ PDF/HTML", data=_html2.encode("utf-8"),
                file_name="Riesgo_"+_rp+"_"+datetime.now().strftime("%Y%m%d")+".html",
                mime="text/html", use_container_width=True, key="risk_html_"+_rp)
        except Exception: st.caption("HTML N/D")

    # ── Tabs del análisis de riesgo ───────────────────────────────────────────
    _rt1, _rt2, _rt3 = st.tabs(["📊 Métricas & Gráficas", "📐 Markowitz Completo", "🌍 Universo de Activos"])
    ...
    # Usar los resultados calculados para ESTE portafolio de riesgo (no el del sidebar)
    _mkz    = _rp_mkz
    _mkz_ok = _rp_mkz_ok
    ...
    if _mkz_ok and _mkz:
        ...
        # ── Usar betas/alphas REALES calculados por run_complete_analysis ────────
        # run_complete_analysis ya ejecutó regresión contra SPX para cada activo
        RF_MK  = float(_mkz.get("rf", 0.0264))
        MRP_MK = float(_mkz.get("MRP", 0.055))   # prima de riesgo real del período

        # Betas y Alfas reales de la regresión (en caso de que estén en el dict)
        _betas_raw  = _mkz.get("Betas")
        _alfas_raw  = _mkz.get("Alfa")
        if _betas_raw is not None and len(_betas_raw) == n_mk:
            betas_mk  = np.array(_betas_raw, dtype=float)
            alphas_mk = np.array(_alfas_raw, dtype=float) if _alfas_raw is not None else np.zeros(n_mk)
        else:
            # Fallback: dict estático si no vienen en el resultado
            _AD_FALLBACK = {
                "NVDA":(1.83,+0.38),"CAT":(1.07,+0.08),"NEM":(0.88,+0.05),
                "AAPL":(1.08,+0.12),"BATS":(0.62,-0.02),"TSLA":(1.65,+0.15),
                "ASTS":(1.43,-0.18),"NOW":(1.27,+0.22),"JPM":(1.13,+0.06),
            ...
            betas_mk  = np.array([_AD_FALLBACK.get(t,(1.0,0.0))[0] for t in tickers_mk])
            alphas_mk = np.array([_AD_FALLBACK.get(t,(1.0,0.0))[1] for t in tickers_mk])

        wts_arr   = np.array([wts_tang.get(t, 0) for t in tickers_mk], dtype=float)
        wts_arr  /= wts_arr.sum()
        ...
        st.markdown("**CAPM por activo** — E(rᵢ) = Rƒ + βᵢ × MRP + αᵢ  *(betas calculados con regresión real vs SPX)*")
        capm_rows = ""
        for _idx_t, t in enumerate(tickers_mk):
            w   = wts_tang.get(t, 0)
            b   = float(betas_mk[_idx_t])
            a   = float(alphas_mk[_idx_t])
        ...
        # ── FRONTERA EFICIENTE (Plotly interactiva) ────────────────────────────
        st.markdown("**Frontera Eficiente** — portafolios de máximo Sharpe y mínima varianza")
        _fr_risks   = _mkz.get("frontier_risks", np.array([]))
        _fr_rets    = _mkz.get("frontier_returns", np.array([]))
        _fr_list    = _mkz.get("frontier", [])
        if len(_fr_risks) > 0:
            fig_fe = go.Figure()
            # Línea de la frontera eficiente
            fig_fe.add_trace(go.Scatter(
                x=_fr_risks * 100, y=_fr_rets * 100,
                mode="lines", line=dict(color="#3b82f6", width=3),
                name="Frontera Eficiente",
                hovertemplate="Vol: %{x:.2f}% · E(r): %{y:.2f}%<extra></extra>"
            ))
            # CML (Capital Market Line) desde Rf hasta tangencia extendida
            _rf_plot = _mkz.get("rf", 0.0264)
            _vt_plot = _mkz["risk_t"] * 100
            _rt_plot = _mkz["ret_t"]  * 100
            _slope_cml = (_rt_plot - _rf_plot * 100) / _vt_plot if _vt_plot > 0 else 0
            _vols_cml = np.linspace(0, _vt_plot * 1.5, 60)
            _rets_cml = _rf_plot * 100 + _slope_cml * _vols_cml
            fig_fe.add_trace(go.Scatter(
                x=_vols_cml, y=_rets_cml,
                mode="lines", line=dict(color="#10b981", width=2, dash="dash"),
                name="CML",
                hovertemplate="CML · Vol: %{x:.2f}%<extra></extra>"
            ))
            # Activos individuales
            _colors_fe = ["#f59e0b","#ef4444","#8b5cf6","#06b6d4","#84cc16","#f43f5e","#3b82f6","#10b981"]
            for _i_fe, _tk_fe in enumerate(tickers_mk):
                _vol_i = float(np.sqrt(_mkz["cov_matrix"][_i_fe, _i_fe])) * 100
                _ret_i = float(_mkz["AssetMean"][_i_fe]) * 100
                fig_fe.add_trace(go.Scatter(
                    x=[_vol_i], y=[_ret_i],
                    mode="markers+text",
                    marker=dict(size=12, color=_colors_fe[_i_fe % len(_colors_fe)],
                                line=dict(width=2, color="white")),
                    text=[_tk_fe], textposition="top right",
                    textfont=dict(size=10, color="#1a2332"),
                    name=_tk_fe,
                    hovertemplate=f"<b>{_tk_fe}</b><br>Vol: %{{x:.2f}}%<br>E(r): %{{y:.2f}}%<extra></extra>"
                ))
            # Portafolio tangencia
            fig_fe.add_trace(go.Scatter(
                x=[_mkz["risk_t"] * 100], y=[_mkz["ret_t"] * 100],
                mode="markers", name="Tangencia ⭐",
                marker=dict(size=18, color="#c9a227", symbol="star",
                            line=dict(width=2, color="white")),
                hovertemplate=f"<b>Tangencia</b><br>Vol: {_mkz['risk_t']*100:.2f}%<br>"
                              f"E(r): {_mkz['ret_t']*100:.2f}%<br>"
                              f"Sharpe: {_mkz['sharpe_t']:.4f}<extra></extra>"
            ))
            # Mínima varianza
            fig_fe.add_trace(go.Scatter(
                x=[_mkz["risk_minvar"] * 100], y=[_mkz["ret_minvar"] * 100],
                mode="markers", name="Mín. Varianza",
                marker=dict(size=14, color="#2d4a6b", symbol="diamond",
                            line=dict(width=2, color="white")),
                hovertemplate=f"<b>Mín. Varianza</b><br>Vol: {_mkz['risk_minvar']*100:.2f}%<br>"
                              f"E(r): {_mkz['ret_minvar']*100:.2f}%<extra></extra>"
            ))
            fig_fe.update_layout(
                ...
                xaxis=dict(title="Volatilidad Anual (%)", showgrid=True,
                           gridcolor="rgba(13,27,42,0.06)"),
                yaxis=dict(title="Retorno Esperado Anual (%)", showgrid=True,
                           gridcolor="rgba(13,27,42,0.06)"),
                legend=dict(orientation="h", yanchor="bottom", y=-0.25,
                            font=dict(size=10)),
                hovermode="closest"
            ...
            st.markdown(
                ...
                "⭐ <b>Tangencia</b> = máximo Sharpe Ratio &nbsp;·&nbsp; "
                "◆ <b>Mínima Varianza</b> = menor riesgo posible &nbsp;·&nbsp; "
                "-- <b>CML</b> = línea de mercado de capitales (Rf → Tangencia)"
                "</div>", unsafe_allow_html=True
            )
            st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

        # ── MONTE CARLO — distribución del retorno anual ───────────────────────
        _sim_pct = _mkz.get("sim_returns_pct")
        if _sim_pct is not None and len(_sim_pct) > 0:
            st.markdown("**Monte Carlo** — distribución del retorno anual del portafolio de tangencia (10,000 simulaciones)")
            _mc_cols = st.columns([2, 1])
            with _mc_cols[0]:
                from scipy.stats import norm as _spnorm
                fig_mc = go.Figure()
                fig_mc.add_trace(go.Histogram(
                    x=_sim_pct, nbinsx=70, histnorm="probability density",
                    marker_color="#3b82f6", opacity=0.75, name="Simulaciones",
                    hovertemplate="Retorno: %{x:.1f}%<extra></extra>"
                ))
                # Curva normal superpuesta
                _mu_mc = float(np.mean(_sim_pct)); _sd_mc = float(np.std(_sim_pct))
                _x_n   = np.linspace(_mu_mc - 4*_sd_mc, _mu_mc + 4*_sd_mc, 200)
                fig_mc.add_trace(go.Scatter(
                    x=_x_n, y=_spnorm.pdf(_x_n, _mu_mc, _sd_mc),
                    mode="lines", line=dict(color="#ef4444", width=2),
                    name="Normal ajustada"
                ))
                fig_mc.add_vline(x=_mkz["ret_p5"], line_dash="dash", line_color="#c0392b",
                    annotation_text=f"VaR 95%: {_mkz['ret_p5']:.1f}%",
                    annotation_font_color="#c0392b", annotation_position="top left")
                fig_mc.add_vline(x=_mkz["ret_p50"], line_dash="dot", line_color="#c9a227",
                    annotation_text=f"Mediana: {_mkz['ret_p50']:.1f}%",
                    annotation_font_color="#c9a227")
                fig_mc.update_layout(
                    height=300, paper_bgcolor="rgba(0,0,0,0)",
                    plot_bgcolor="rgba(232,240,248,0.25)",
                    font=dict(family="DM Sans"), margin=dict(l=0,r=0,t=20,b=0),
                    xaxis=dict(title="Retorno anual (%)", showgrid=False),
                    yaxis=dict(title="Densidad", showgrid=True, gridcolor="#eef2f7"),
                    showlegend=False
                )
                st.markdown('<div class="chart3d">', unsafe_allow_html=True)
                ...
                st.markdown('</div>', unsafe_allow_html=True)
            with _mc_cols[1]:
                _p_loss = _mkz.get("prob_perdida", 0.0)
                _p_col  = "#1a7a4a" if _p_loss < 25 else "#d4820a" if _p_loss < 40 else "#c0392b"
                st.markdown(f"""
                <div style='background:white;border-radius:14px;padding:20px;
                            box-shadow:0 2px 12px rgba(13,27,42,0.07);text-align:center;'>
                    <div style='font-size:10px;font-weight:700;color:#6b7c93;
                                text-transform:uppercase;margin-bottom:12px;'>Percentiles</div>
                    <div style='display:flex;flex-direction:column;gap:6px;font-size:12px;'>
                        <div style='display:flex;justify-content:space-between;'>
                            <span style='color:#6b7c93;'>P5 (VaR)</span>
                            <b style='color:#c0392b;'>{_mkz['ret_p5']:.1f}%</b></div>
                        <div style='display:flex;justify-content:space-between;'>
                            <span style='color:#6b7c93;'>P25</span>
                            <b style='color:#d4820a;'>{_mkz['ret_p25']:.1f}%</b></div>
                        <div style='display:flex;justify-content:space-between;'>
                            <span style='color:#6b7c93;'>P50 (Mediana)</span>
                            <b style='color:#1a3a5c;'>{_mkz['ret_p50']:.1f}%</b></div>
                        <div style='display:flex;justify-content:space-between;'>
                            <span style='color:#6b7c93;'>P75</span>
                            <b style='color:#1a7a4a;'>{_mkz['ret_p75']:.1f}%</b></div>
                        <div style='display:flex;justify-content:space-between;'>
                            <span style='color:#6b7c93;'>P95</span>
                            <b style='color:#1a7a4a;'>{_mkz['ret_p95']:.1f}%</b></div>
                    </div>
                    <hr style='border-color:#eef2f7;margin:14px 0;'>
                    <div style='font-size:11px;color:#6b7c93;margin-bottom:4px;'>Prob. pérdida anual</div>
                    <div style='font-family:"DM Serif Display",serif;font-size:32px;
                                color:{_p_col};'>{_p_loss:.1f}%</div>
                    <div style='font-size:11px;color:{_p_col};font-weight:600;'>
                        {"Bajo riesgo ✅" if _p_loss < 25 else "Moderado ⚠️" if _p_loss < 40 else "Alto riesgo ⛔"}
                    </div>
                </div>""", unsafe_allow_html=True)
            st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
    ...
    if not alerts_df.empty:
        ...
        fig_sml.update_layout(
            height=420,
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.25)",
            font=dict(family="DM Sans", color="#1a2332"),
    ...
    fig_comp.update_layout(
        height=400, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.25)",
        font=dict(family="DM Sans", color="#1a2332"),
        margin=dict(l=0, r=0, t=10, b=0),
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        "<div style='font-size:12px;color:#4a5568;background:#fffbea;border:1px solid #f9d100;"
        "border-radius:8px;padding:10px 14px;margin-bottom:10px;'>"
        "📧 <b>Notificaciones email:</b> agrega <code>\"email\":\"correo@dominio.com\"</code> "
        "a cada usuario en el bloque <code>USERS</code> del código, y configura "
        "<code>[email]</code> en Streamlit Secrets (smtp_host, smtp_port, smtp_user, smtp_pass)."
        "</div>", unsafe_allow_html=True
    )
    ...
    with st.expander("➕ Crear Nuevo Portafolio"):
        st.caption("Leerá hojas Precios_NOMBRE y Ops_NOMBRE del Sheet configurado.")
        _nc1, _nc2, _nc3 = st.columns([2, 2, 1])
        with _nc1: _new_pname = st.text_input("Nombre", placeholder="Moderado", key="new_pname")
        with _nc2: _new_pdesc = st.text_input("Descripción", placeholder="Balance", key="new_pdesc")
        with _nc3: _new_pcolor = st.color_picker("Color", "#6366f1", key="new_pcolor")
        _new_puniv = st.text_input("Tickers (coma-separados)", placeholder="NVDA, AAPL, MSFT", key="new_puniv")
        _new_pwts  = st.text_input("Pesos % semilla (deben sumar 100)", placeholder="50, 30, 20", key="new_pwts")
        if "custom_portfolios" not in st.session_state:
            st.session_state.custom_portfolios = {}
        if st.button("➕ Crear", key="btn_create_port"):
            _n  = _new_pname.strip()
            _u  = [t.strip().upper() for t in _new_puniv.split(",") if t.strip()]
            _wr = [w.strip() for w in _new_pwts.split(",") if w.strip()]
            if not _n: st.error("Nombre requerido.")
            elif _n in get_demo_portfolios(): st.error(f"Ya existe '{_n}'.")
            elif not _u: st.error("Agrega tickers.")
            else:
                try:
                    ...
                    st.session_state.custom_portfolios[_n] = {
                        "description": _new_pdesc.strip() or _n,
                        "color": _new_pcolor, "rf": 0.0264,
                        "universe": _u, "target": _wn}
                    st.success(f"Portafolio '{_n}' creado.")
                    st.rerun()
                except Exception as _ex: st.error(f"Error: {_ex}")
        for _pn, _pd in list(st.session_state.get("custom_portfolios",{}).items()):
            _pp1, _pp2 = st.columns([5,1])
            with _pp1: st.markdown(f"<div style='background:#f0f4f8;border-left:4px solid {_pd['color']};border-radius:6px;padding:7px 12px;font-size:13px;'><b>{_pn}</b> — {_pd.get('description','')} · {len(_pd.get('universe',[]))} tickers</div>", unsafe_allow_html=True)
            with _pp2:
                if st.button("🗑", key=f"del_{_pn}"): del st.session_state.custom_portfolios[_pn]; st.rerun()

# PAGE: QUICKVIEW (admin only)
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    st.markdown("<div class='section-title'>🔭 QuickView</div>", unsafe_allow_html=True)
    all_ports = get_demo_portfolios()
    _qv_opts = ["Todos"] + [v.get("name", k) for k, v in USERS.items()]
    if "qv_sel_user" not in st.session_state:
        st.session_state.qv_sel_user = "Todos"
    _qvc1, _qvc2 = st.columns([1, 3])
    with _qvc1:
        _qvsel = st.selectbox("Filtrar", _qv_opts,
            index=_qv_opts.index(st.session_state.qv_sel_user)
            if st.session_state.qv_sel_user in _qv_opts else 0,
            key="qv_usr_sel")
        st.session_state.qv_sel_user = _qvsel
    _qv_fk = None if _qvsel == "Todos" else next(
        (k for k, v in USERS.items() if v.get("name", k) == _qvsel), None)
    _qv_list = [(k, v) for k, v in USERS.items()
                if _qv_fk is None or k == _qv_fk]
    for uname, udata in _qv_list:
        uports = udata.get("portfolios") or list(all_ports.keys())
        if not uports: continue
        st.markdown(f"<div style='background:#1a3a5c;color:white;border-radius:12px;padding:10px 18px;margin-top:20px;'><b>{udata.get('name',uname)}</b> · {udata.get('role','investor').upper()} · {len(uports)} portafolio(s)</div>", unsafe_allow_html=True)
        for pname in uports:
            port_info=all_ports.get(pname,{});u_tgt=port_info.get("target",{})
            _uid_own=udata.get("sheet_id","").strip()
            uid_sheet=_uid_own if _uid_own else sheet_id
            u_ops=pd.DataFrame()
                ...
                try:
                    ...
                    if _raw is not None and not _raw.empty:
                        if "Usuario" in _raw.columns:
                            _un=udata.get("name",uname).lower()
                            _uc=_raw["Usuario"].astype(str).str.strip().str.lower()
                            _msk=(_uc==uname.lower())|(_uc==_un)|(_uc==_un.split()[0])
                            _f=_raw[_msk].copy()
                            u_ops=_f if not _f.empty else _raw
                        else: u_ops=_raw
                except: pass
            if u_ops.empty: u_ops=get_demo_operations(pname)
            _u_prices={}
            if not u_ops.empty and "Precio_USD" in u_ops.columns:
                _u_prices=u_ops.sort_values("Fecha").groupby("Ticker")["Precio_USD"].last().to_dict()
            u_pos=calc_positions(u_ops,_u_prices,tc_actual)
            ...
            n_ops=len(u_ops);u_rm={}
            if uid_sheet and use_sheets and u_tgt:
                try:
                    u_ret_df=load_price_history_from_sheets(uid_sheet,pname)
                    if u_ret_df is not None: u_rm=calc_risk_metrics_bloomberg(u_ret_df,u_tgt)
                except: pass
            sharpe_u=u_rm.get("sharpe",0);ret_u=u_rm.get("ret_annual",0);vol_u=u_rm.get("vol_annual",0)
            sc="#1a7a4a" if sharpe_u>=1.0 else "#d4820a" if sharpe_u>=0.5 else "#c0392b"
            wts_str=" · ".join([f"{t} {w*100:.0f}%" for t,w in list(u_tgt.items())[:5]])
            _first_date = "";_rendimiento_pct = 0.0
            if not u_ops.empty and "Fecha" in u_ops.columns:
                try:
                    _fd = pd.to_datetime(u_ops["Fecha"]).min();_first_date = _fd.strftime("%d %b %Y")
                    _cost = (u_ops[u_ops["Tipo"]=="Compra"]["Cantidad"]*u_ops[u_ops["Tipo"]=="Compra"]["Precio_USD"]).sum()
                    if _cost > 0 and total_usd_u > 0: _rendimiento_pct = (total_usd_u-_cost)/_cost*100
                except: pass
            _rc="#1a7a4a" if _rendimiento_pct>=0 else "#c0392b"
            _rs="+" if _rendimiento_pct>=0 else ""
            c1,c2,c3,c4,c5=st.columns(5)
            with c1: st.markdown(f"<div class='kpi-card' style='padding:12px 14px;'><div class='kpi-label'>📁 {pname}</div><div class='kpi-value num-anim' style='font-size:18px;color:#1a3a5c;'>${total_mxn_u:,.0f}</div><div class='kpi-sub'>MXN</div></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div class='kpi-card' style='padding:12px 14px;'><div class='kpi-label'>📈 Rendimiento</div><div class='kpi-value num-anim' style='font-size:18px;color:{_rc};'>{_rs}{_rendimiento_pct:.1f}%</div><div class='kpi-sub'>Desde {_first_date or 'inicio'}</div></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div class='kpi-card' style='padding:12px 14px;'><div class='kpi-label'>📋 Ops</div><div class='kpi-value num-anim' style='font-size:18px;color:#1a3a5c;'>{n_ops}</div><div class='kpi-sub'>registradas</div></div>", unsafe_allow_html=True)
            with c4: st.markdown(f"<div class='kpi-card' style='padding:12px 14px;'><div class='kpi-label'>⚡ Sharpe</div><div class='kpi-value num-anim' style='font-size:18px;color:{sc};'>{sharpe_u:.2f}</div><div class='kpi-sub'>Ret {ret_u*100:.1f}% · Vol {vol_u*100:.1f}%</div></div>", unsafe_allow_html=True)
            with c5: st.markdown(f"<div class='kpi-card' style='padding:12px 14px;'><div class='kpi-label'>🎯 Pesos</div><div style='font-size:11px;color:#1a3a5c;line-height:1.8;margin-top:4px;'>{wts_str}</div></div>", unsafe_allow_html=True)
