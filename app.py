...
@media(max-width:480px){
    ...
    /* Sidebar collapsed toggle — visible on mobile */
    [data-testid="stSidebarCollapsedControl"]{
        display:flex !important;visibility:visible !important;
        opacity:1 !important;pointer-events:auto !important;
        position:fixed !important;top:10px !important;left:10px !important;
        z-index:999999 !important;
        background:linear-gradient(135deg,rgba(13,27,42,0.95),rgba(26,58,92,0.9)) !important;
    ...
    [data-testid="stSidebarCollapsedControl"] svg{
        fill:#c9a227 !important;stroke:#c9a227 !important;
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    ...
    if not _is_admin:
        ...
        # ── Download buttons ──────────────────────────────────────────────
        _dcol1, _dcol2, _dspacer = st.columns([1, 1, 5])
        with _dcol1:
            try:
                _xl_buf  = build_portfolio_excel(selected_port, port_data,
                               positions_df, history, alerts_df,
                               risk_metrics, target_wts, tc_actual)
                st.download_button("⬇ Excel", data=_xl_buf,
                    file_name=f"Reporte_{selected_port}_{datetime.now().strftime('%Y%m%d')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True, key=f"dl_xl_{selected_port}")
            except Exception: st.caption("Excel N/D")
        with _dcol2:
            try:
                _sh = risk_metrics.get("sharpe",0)
                _re = risk_metrics.get("ret_annual",0)*100
                _vo = risk_metrics.get("vol_annual",0)*100
                _va = abs(risk_metrics.get("var_95",0))*100
                _pos_tbl = "".join(
                    f"<tr><td><b>{r.get('Ticker','')}</b></td>"
                    f"<td>{r.get('Acciones',0):.1f}</td>"
                    f"<td>${r.get('Precio USD',0):,.2f}</td>"
                    f"<td>${r.get('Valor MXN',0):,.0f}</td></tr>"
                    for _, r in positions_df.iterrows()) if not positions_df.empty else ""
                _al_tbl = "".join(
                    f"<tr style='background:#fff9c4'><td><b>{r.get('Ticker','')}</b></td>"
                    f"<td style='color:{'#1a7a4a' if r.get('Alerta')=='COMPRAR' else '#c0392b'};font-weight:700'>{r.get('Alerta','')}</td>"
                    f"<td>{r.get('Actual %',r.get('w_actual',''))}</td>"
                    f"<td>{r.get('Objetivo %',r.get('w_obj',''))}</td></tr>"
                    for _, r in alerts_df[alerts_df["Alerta"].isin(["COMPRAR","VENDER"])].iterrows()
                ) if not alerts_df.empty else ""
                _wts_tbl = "".join(
                    f"<tr><td><b>{t}</b></td><td>{w*100:.1f}%</td></tr>"
                    for t, w in target_wts.items())
                _html_r = f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<style>body{{font-family:Arial,sans-serif;background:#edf2f8;margin:0;}}
.page{{max-width:860px;margin:24px auto;background:white;border-radius:14px;
       box-shadow:0 4px 20px rgba(0,0,0,.10);overflow:hidden;}}
.hdr{{background:linear-gradient(135deg,#0d2240,#1a3a5c);padding:28px 36px;}}
.hdr h1{{color:#c9a227;font-size:26px;margin:0;letter-spacing:.02em;}}
.hdr p{{color:#9ab0c8;font-size:12px;margin:4px 0 0;}}
.body{{padding:28px 36px;}}
.kpis{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0;}}
.kpi{{background:#edf2f8;border-radius:10px;padding:12px;text-align:center;}}
.kpi .v{{font-size:22px;font-weight:800;color:#1a3a5c;}}
.kpi .l{{font-size:10px;color:#6b7c93;margin-top:2px;}}
table{{width:100%;border-collapse:collapse;font-size:12px;margin:10px 0;}}
th{{background:#1a3a5c;color:#fff;padding:9px 12px;text-align:left;}}
td{{padding:8px 12px;border-bottom:1px solid #f0f4f9;}}
tr:nth-child(even) td{{background:#f8fafc;}}
.sec{{font-size:12px;font-weight:700;color:#1a3a5c;text-transform:uppercase;
      letter-spacing:.08em;margin:22px 0 8px;padding-bottom:4px;
      border-bottom:2px solid #c9a227;}}
.alr{{background:#f9d100;border-radius:8px;padding:12px 16px;margin:14px 0;}}
.alr b{{font-size:15px;color:#1a1400;}}
.alr span{{font-size:11px;color:#5c4d00;}}
.ftr{{background:#f8fafc;padding:14px 36px;font-size:10px;color:#9ab0c8;
      border-top:1px solid #e8eef5;}}
</style></head><body><div class="page">
<div class="hdr"><h1>Portafolio {selected_port}</h1>
<p>Reporte Ejecutivo · {datetime.now().strftime("%d %b %Y %H:%M")} · EGADE Portfolio Manager</p></div>
<div class="body">
{'<div class="alr"><b>⚠ Tu portafolio necesita ajuste</b><br><span>Revisa las alertas abajo</span></div>' if _al_tbl else ""}
<div class="kpis">
  <div class="kpi"><div class="v">{_sh:.2f}</div><div class="l">Sharpe Ratio</div></div>
  <div class="kpi"><div class="v">{_re:+.1f}%</div><div class="l">Retorno Anual</div></div>
  <div class="kpi"><div class="v">{_vo:.1f}%</div><div class="l">Volatilidad</div></div>
  <div class="kpi"><div class="v">{_va:.1f}%</div><div class="l">VaR 95%</div></div>
</div>
<div class="sec">Posiciones Actuales</div>
<table><thead><tr><th>Ticker</th><th>Acciones</th><th>Precio USD</th><th>Valor MXN</th></tr></thead>
<tbody>{_pos_tbl}</tbody></table>
{'<div class="sec">Alertas de Rebalanceo</div><table><thead><tr><th>Activo</th><th>Acción</th><th>Actual</th><th>Objetivo</th></tr></thead><tbody>' + _al_tbl + '</tbody></table>' if _al_tbl else ""}
<div class="sec">Pesos Objetivo Markowitz</div>
<table><thead><tr><th>Ticker</th><th>Peso Objetivo</th></tr></thead>
<tbody>{_wts_tbl}</tbody></table>
<div class="sec">Guía de Interpretación</div>
<p style="font-size:12px;color:#4a5568;line-height:1.7;">
<b>Sharpe Ratio:</b> Mide retorno por unidad de riesgo. &gt;1.0 = sólido, &gt;1.5 = excelente.<br>
<b>Retorno Anual:</b> Rentabilidad promedio anualizada del portafolio con pesos objetivo.<br>
<b>Volatilidad:</b> Desviación estándar anualizada — mide la dispersión de retornos.<br>
<b>VaR 95%:</b> Pérdida máxima esperada en el 95% de los escenarios mensuales.<br>
<b>Alertas COMPRAR:</b> El activo está subponderado — su peso actual es menor al objetivo.<br>
<b>Alertas VENDER:</b> El activo está sobreponderado — su peso actual supera al objetivo.<br>
<b>Rebalanceo:</b> Ajustar posiciones para volver a los pesos óptimos de Markowitz.
</p></div>
<div class="ftr">EGADE Business School · Mercados Financieros · Portfolio Manager · {datetime.now().year}</div>
</div></body></html>"""
                st.download_button("⬇ PDF/HTML", data=_html_r.encode("utf-8"),
                    file_name=f"Reporte_{selected_port}_{datetime.now().strftime('%Y%m%d')}.html",
                    mime="text/html", use_container_width=True,
                    key=f"dl_html_{selected_port}")
...
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    ...
    for uname, udata in _qv_list:
        ...
        for pname in uports:
            ...
            c1,c2,c3,c4=st.columns(4)
            with c1: st.markdown(f"<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;border-radius:8px;'><div style='font-size:11px;color:#6b7c93;font-weight:700;'>📁 {pname}</div><div style='font-size:20px;font-weight:800;color:#1a3a5c;'>${total_mxn_u:,.0f}</div><div style='font-size:11px;color:#6b7c93;'>MXN · ${total_usd_u:,.0f} USD</div></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;border-radius:8px;'><div style='font-size:11px;color:#6b7c93;font-weight:700;'>📋 Operaciones</div><div style='font-size:20px;font-weight:800;color:#1a3a5c;'>{n_ops}</div></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;border-radius:8px;'><div style='font-size:11px;color:#6b7c93;font-weight:700;'>⚡ Sharpe</div><div style='font-size:20px;font-weight:800;color:{sc};'>{sharpe_u:.2f}</div><div style='font-size:11px;color:#6b7c93;'>Ret {ret_u*100:.1f}% · Vol {vol_u*100:.1f}%</div></div>", unsafe_allow_html=True)
            with c4: st.markdown(f"<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;border-radius:8px;'><div style='font-size:11px;color:#6b7c93;font-weight:700;'>🎯 Pesos</div><div style='font-size:11px;color:#1a3a5c;line-height:1.8;'>{wts_str}</div></div>", unsafe_allow_html=True)
