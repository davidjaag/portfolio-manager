...
YF_TICKER_MAP = {
    "NVDA": "NVDA", "CAT": "CAT", "NEM": "NEM", "AAPL": "AAPL",
    "BATS": "BATS.L", "TSLA": "TSLA", "ASTS": "ASTS", "NOW": "NOW",
    "JPM": "JPM", "9988HK": "9988.HK", "SPX": "^GSPC",
    "GLD": "GLD", "IAU": "IAU", "GE": "GE", "CIEN": "CIEN",
}
...
def get_base_portfolios() -> dict:
    """
    4 portafolios base. 'target' son pesos semilla (sobreescritos por modelo en runtime).
    Cada portafolio opera como universo completamente independiente.
    """
    return {
        "Conservador": {
            ...
            "rf": 0.0264,
            "profile": "conservador",
            "universe": ["NEM", "BATS", "CAT", "AAPL", "JPM"],
            "target": {"NEM": 0.35, "BATS": 0.30, "CAT": 0.20, "AAPL": 0.10, "JPM": 0.05},
            "objetivo": "Preservar capital con baja volatilidad",
            "horizonte": "1–4 años",
            "tolerancia": "Baja",
            "tolerancia_pct": 20,
            "horizonte_years": 3,
            "volatilidad_range": (8, 14),
            "retorno_obj": (10, 16),
            "sharpe_obj": 0.8,
            "icon": "🛡️",
        ...
        "Moderado": {
            ...
            "rf": 0.0264,
            "profile": "moderado",
            "universe": ["NVDA", "CAT", "NEM", "AAPL", "JPM"],
            "target": {"NVDA": 0.30, "CAT": 0.25, "NEM": 0.20, "AAPL": 0.15, "JPM": 0.10},
            "objetivo": "Equilibrio entre crecimiento y estabilidad",
            "horizonte": "3–7 años",
            "tolerancia": "Media",
            "tolerancia_pct": 50,
            "horizonte_years": 5,
            "volatilidad_range": (15, 25),
            "retorno_obj": (18, 28),
            "sharpe_obj": 1.0,
            "icon": "⚖️",
        ...
        "Balanceado": {
            ...
            "rf": 0.0264,
            "profile": "balanceado",
            "universe": ["NVDA", "CAT", "NEM", "AAPL", "TSLA", "JPM"],
            "target": {"NVDA": 0.30, "CAT": 0.20, "NEM": 0.15, "AAPL": 0.15, "TSLA": 0.10, "JPM": 0.10},
            "objetivo": "Maximizar Sharpe Ratio — máxima eficiencia",
            "horizonte": "5–10 años",
            "tolerancia": "Media-Alta",
            "tolerancia_pct": 60,
            "horizonte_years": 7,
            "volatilidad_range": (20, 30),
            "retorno_obj": (25, 40),
            "sharpe_obj": 1.2,
            "icon": "📊",
        ...
        "AltoRendimiento": {
            ...
            "rf": 0.0264,
            "profile": "agresivo",
            "universe": ["NVDA", "TSLA", "ASTS", "NOW", "AAPL"],
            "target": {"NVDA": 0.30, "TSLA": 0.25, "ASTS": 0.20, "NOW": 0.15, "AAPL": 0.10},
            "objetivo": "Maximizar retorno absoluto con alta concentración",
            "horizonte": "5–10 años",
            "tolerancia": "Alta",
            "tolerancia_pct": 80,
            "horizonte_years": 8,
            "volatilidad_range": (35, 55),
            "retorno_obj": (50, 80),
            "sharpe_obj": 1.0,
            "icon": "🚀",
...
def get_demo_ops(portfolio_name: str) -> pd.DataFrame:
    ops_map = {
        "Conservador": [
            {"Fecha": "2025-01-15", "Ticker": "NEM",  "Tipo": "Compra", "Cantidad": 30.0, "Precio_USD": 41.00, "Comision_USD": 0.50, "TC_MXN": 20.05},
            {"Fecha": "2025-01-15", "Ticker": "BATS", "Tipo": "Compra", "Cantidad": 25.0, "Precio_USD": 34.80, "Comision_USD": 0.50, "TC_MXN": 20.05},
            {"Fecha": "2025-01-15", "Ticker": "CAT",  "Tipo": "Compra", "Cantidad": 2.0,  "Precio_USD": 350.00,"Comision_USD": 0.50, "TC_MXN": 20.05},
            {"Fecha": "2025-06-01", "Ticker": "NEM",  "Tipo": "Compra", "Cantidad": 10.0, "Precio_USD": 50.20, "Comision_USD": 0.50, "TC_MXN": 20.00},
        ],
        "Moderado": [
            {"Fecha": "2025-01-20", "Ticker": "NVDA", "Tipo": "Compra", "Cantidad": 8.0,  "Precio_USD": 135.00,"Comision_USD": 0.50, "TC_MXN": 20.10},
            {"Fecha": "2025-01-20", "Ticker": "CAT",  "Tipo": "Compra", "Cantidad": 3.0,  "Precio_USD": 358.00,"Comision_USD": 0.50, "TC_MXN": 20.10},
            {"Fecha": "2025-01-20", "Ticker": "NEM",  "Tipo": "Compra", "Cantidad": 20.0, "Precio_USD": 44.80, "Comision_USD": 0.50, "TC_MXN": 20.10},
            {"Fecha": "2025-06-01", "Ticker": "NVDA", "Tipo": "Compra", "Cantidad": 3.0,  "Precio_USD": 131.00,"Comision_USD": 0.50, "TC_MXN": 19.80},
        ],
        "Balanceado": [
...
def get_demo_prices() -> dict:
    return {
        ...
        "JPM": 238.00, "9988HK": 95.00, "SPX": 5720.00, "GLD": 185.00,
    }
...
# Pesos objetivos: del modelo si disponible, sino semilla de config
if model_result and model_result.get("w_tang"):
    ...
    _wt_source = "Modelo Markowitz (datos reales)"
else:
    target_wts = port_data.get("target", {})
    _wt_source = "Pesos semilla (sin datos históricos)"
...
# ─── HELPER: PROFILE CARD ────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
def render_profile_card(port_data_dict: dict, selected_port_name: str):
    ...
    sobj = port_data_dict.get("sharpe_obj", 1.0)
    ...
    <div style='background:{bg};border-radius:16px;padding:20px 24px;
        ...
        <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;'>
            ...
            <div style='background:rgba(255,255,255,0.75);border-radius:11px;padding:12px;'>
                ...
                <div style='font-size:10px;color:#6b7c93;margin-top:2px;'>Sharpe objetivo: &gt; {sobj}</div>
            </div>
        </div>
    </div>""", unsafe_allow_html=True)
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    render_profile_card(port_data, selected_port)
    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

    # ── KPI Row ───────────────────────────────────────────────────────────────
    k1, k2, k3, k4 = st.columns(4)
    sign = "+" if total_ganancia >= 0 else ""
    gc = "positive" if total_ganancia >= 0 else "negative"
    sr_label, sr_color = sharpe_label(risk_metrics.get("sharpe", 0))

    with k1:
        st.markdown(f"""<div class="kpi-card">
            <div class="kpi-label">Valor del Portafolio</div>
            <div class="kpi-value">${total_mxn:,.0f}</div>
            <div class="kpi-sub">MXN · equivale a ${total_usd:,.0f} USD · TC {tc_actual:.2f}</div>
        </div>""", unsafe_allow_html=True)
    with k2:
        st.markdown(f"""<div class="kpi-card">
            <div class="kpi-label">Ganancia / Pérdida</div>
            <div class="kpi-value {gc}">{sign}${total_ganancia:,.0f} USD</div>
            <div class="kpi-sub">{sign}{gan_pct:.1f}% sobre lo invertido</div>
        </div>""", unsafe_allow_html=True)
    with k3:
        st.markdown(f"""<div class="kpi-card">
            <div class="kpi-label">Sharpe Ratio (modelo)</div>
            <div class="kpi-value" style="color:{sr_color};">{risk_metrics.get('sharpe',0):.3f}</div>
            <div class="kpi-sub">{sr_label}</div>
        </div>""", unsafe_allow_html=True)
    with k4:
        vol = risk_metrics.get("vol_annual", 0)
        vlo_, vhi_ = port_data.get("volatilidad_range", (15, 25))
        vc = "#1a7a4a" if vol*100 <= vhi_ else "#c0392b"
        st.markdown(f"""<div class="kpi-card">
            <div class="kpi-label">Volatilidad Anual</div>
            <div class="kpi-value" style="color:{vc};">{vol*100:.1f}%</div>
            <div class="kpi-sub">Objetivo: {vlo_}%–{vhi_}% · {"✅ En rango" if vol*100<=vhi_ else "⚠ Fuera de rango"}</div>
        </div>""", unsafe_allow_html=True)
    ...
    _vth = port_data.get("volatilidad_range", (15, 25))[1] / 100

    sem_cols = st.columns(4)
    for _col, _ico_g, _ico_b, _lbl, _val_s, _txt_g, _txt_b, _is_good in [
        (sem_cols[0], "🟢", "🔴", "Eficiencia (Sharpe)", f"{_sr:.2f}",
         "Excelente — portafolio eficiente", "Bajo — revisa estrategia", _sr >= 1.0),
        (sem_cols[1], "🟢", "🟡", "Volatilidad Anual", f"{_vol*100:.1f}%",
         "Estable para tu perfil", "Más volátil de lo esperado", _vol <= _vth),
        (sem_cols[2], "🟢", "🔴", "Caída Máxima", f"{_dd*100:.1f}%",
         "Drawdown controlado", "Caída importante — evalúa", _dd >= -0.15),
        (sem_cols[3], "🟢", "🟡", "VaR 95% mensual", f"{abs(_var)*100:.1f}%",
         "Riesgo mensual bajo", "Riesgo mensual elevado", abs(_var) <= 0.05),
    ]:
        ico = _ico_g if _is_good else _ico_b
        color = "#1a7a4a" if _is_good else "#c0392b"
        txt = _txt_g if _is_good else _txt_b
        with _col:
            st.markdown(f"""
            <div style='background:white;border-radius:12px;padding:14px;text-align:center;
                ...
                <div style='font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#6b7c93;margin-bottom:3px;'>{_lbl}</div>
                <div style='font-family:"DM Serif Display",serif;font-size:22px;color:{color};'>{_val_s}</div>
                <div style='font-size:10px;color:{color};font-weight:600;margin-top:3px;'>{txt}</div>
            </div>""", unsafe_allow_html=True)
    ...
    if has_actions:
        ...
        st.markdown(
            ...
            "El modelo Markowitz detectó desviaciones respecto a los pesos óptimos. "
            "Estas operaciones restauran la eficiencia del portafolio.</div>",
            unsafe_allow_html=True)
        order_cols = st.columns(min(len(pending), 4))
        for i, (_, row) in enumerate(pending.iterrows()):
            col_idx = i % len(order_cols)
            with order_cols[col_idx]:
                if row["Alerta"] == "COMPRAR":
                    mxn_c = row["Compra Sugerida MXN"]
                    usd_c = mxn_c / tc_actual
                    st.markdown(
                        "<div class='order-card order-buy'>"
                        "<div style='font-size:24px'>🟢</div>"
                        "<div style='flex:1'>"
                        f"<div style='font-weight:700;font-size:15px;color:#1a3a5c;'>{row['Ticker']}</div>"
                        "<div style='font-size:11px;font-weight:700;color:#1a7a4a;margin-bottom:5px;'>COMPRAR</div>"
                        f"<div style='font-size:12px;'><b>{row['Acciones a Comprar']:.4f} acc</b> @ ${row['Precio Actual']:,.2f}</div>"
                        f"<div style='font-size:12px;color:#1a7a4a;'><b>${usd_c:,.2f} USD</b> · ${mxn_c:,.0f} MXN</div>"
                        f"<div style='font-size:10px;color:#888;margin-top:3px;'>"
                        f"Tienes {row['Peso Actual']*100:.1f}% → Obj {row['Peso Objetivo']*100:.1f}%</div>"
                        "</div></div>", unsafe_allow_html=True)
                else:
                    mxn_v = row["Monto a Vender MXN"]
                    usd_v = mxn_v / tc_actual
                    st.markdown(
                        "<div class='order-card order-sell'>"
                        "<div style='font-size:24px'>🔴</div>"
                        "<div style='flex:1'>"
                        f"<div style='font-weight:700;font-size:15px;color:#1a3a5c;'>{row['Ticker']}</div>"
                        "<div style='font-size:11px;font-weight:700;color:#c0392b;margin-bottom:5px;'>VENDER</div>"
                        f"<div style='font-size:12px;'><b>{row['Acciones a Vender']:.4f} acc</b> @ ${row['Precio Actual']:,.2f}</div>"
                        f"<div style='font-size:12px;color:#c0392b;'><b>${usd_v:,.2f} USD</b> · ${mxn_v:,.0f} MXN</div>"
                        f"<div style='font-size:10px;color:#888;margin-top:3px;'>"
                        f"Tienes {row['Peso Actual']*100:.1f}% → Obj {row['Peso Objetivo']*100:.1f}%</div>"
                        "</div></div>", unsafe_allow_html=True)
    else:
        st.success("✅ Portafolio en equilibrio — no hay acciones pendientes según el modelo.")

    # ── Cotizaciones en tiempo real ───────────────────────────────────────────
    st.markdown("<div class='section-title'>📡 Cotizaciones en Tiempo Real</div>",
                unsafe_allow_html=True)
    if _yf_prices:
        val_cols = st.columns(min(len(target_wts), 5))
        for i, tkr in enumerate(target_wts.keys()):
            q = get_live_quotes(list(target_wts.keys())).get(tkr, {})
            price = q.get("price")
            chg = q.get("change_pct")
            w52h = q.get("week52_high")
            w52l = q.get("week52_low")
            vs_ma = q.get("vs_ma52w")
            with val_cols[i % len(val_cols)]:
                if not price:
                    st.markdown(f"<div style='background:white;border-radius:12px;padding:14px;"
                               f"text-align:center;border:1px solid #e8eef5;'><b>{tkr}</b><br>"
                               f"<span style='color:#aab;font-size:12px;'>Sin datos</span></div>",
                               unsafe_allow_html=True)
                    continue
                cc = "#1a7a4a" if (chg or 0) >= 0 else "#c0392b"
                cbg = "#d5f5e3" if (chg or 0) >= 0 else "#fadbd8"
                cico = "▲" if (chg or 0) >= 0 else "▼"
                cstr = f"{cico} {abs(chg):.2f}%" if chg is not None else "—"
                # Signal vs 52w avg
                if vs_ma is None: sig, sc, sbg = "Sin datos", "#6b7c93", "#f4f7fb"
                elif vs_ma > 20: sig, sc, sbg = "MUY CARA 🔴", "#c0392b", "#fadbd8"
                elif vs_ma > 5: sig, sc, sbg = "ALTA 🟡", "#d4820a", "#fef9e7"
                elif vs_ma >= -5: sig, sc, sbg = "JUSTO 🔵", "#2d4a6b", "#e8f0f8"
                elif vs_ma >= -20: sig, sc, sbg = "BAJA 🟢", "#1a7a4a", "#d5f5e3"
                else: sig, sc, sbg = "MUY BAJA 🟢", "#1a7a4a", "#d5f5e3"
                bar_p = max(0, min(100, (price - (w52l or 0)) / ((w52h or price+1) - (w52l or 0)) * 100)) if w52h and w52l else 50
                st.markdown(
                    ...
                    f"<span style='font-weight:700;font-size:14px;'>{tkr}</span>"
                    f"<span style='background:{cbg};color:{cc};padding:2px 7px;"
                    f"border-radius:9px;font-size:10px;font-weight:700;'>{cstr}</span></div>"
                    f"<div style='font-size:24px;font-weight:700;color:#1a3a5c;margin-bottom:8px;'>${price:,.2f}</div>"
                    f"<div style='background:{sbg};border-radius:7px;padding:5px 9px;margin-bottom:8px;"
                    f"border-left:3px solid {sc};'>"
                    f"<div style='font-size:10px;font-weight:700;color:{sc};'>{sig}</div>"
                    f"<div style='font-size:9px;color:#6b7c93;'>{f'{vs_ma:+.1f}% vs prom 52s' if vs_ma else ''}</div></div>"
                    f"<div style='font-size:9px;color:#6b7c93;margin-bottom:3px;'>52s: ${w52l:,.2f}–${w52h:,.2f}</div>"
                    f"<div style='background:#eef2f7;border-radius:4px;height:5px;position:relative;'>"
                    f"<div style='position:absolute;left:{bar_p:.0f}%;top:-4px;width:13px;height:13px;"
        ...
        st.info("📡 Yahoo Finance no disponible — instala yfinance para cotizaciones en tiempo real.")

    # ── Posiciones + Donuts ───────────────────────────────────────────────────
    st.markdown("<div class='section-title'>📋 Posiciones Actuales</div>", unsafe_allow_html=True)
    ...
    with col_left:
        if not alerts_df.empty:
            ...
            for _, row in alerts_df.iterrows():
                gc2 = "#1a7a4a" if row["Ganancia USD"] >= 0 else "#c0392b"
                gs = "+" if row["Ganancia USD"] >= 0 else ""
                rows_html += (
                    f"<tr><td><b>{row['Ticker']}</b></td>"
                    f"<td>{row['Cantidad']:.4f}</td>"
                    f"<td>${row['Precio Actual']:,.2f}</td>"
                    f"<td><b>${row['Valor MXN']:,.0f}</b></td>"
                    f"<td>{row['Peso Actual']*100:.1f}%</td>"
                    f"<td>{row['Peso Objetivo']*100:.1f}%</td>"
                    "<td style='color:" + ("#c0392b" if abs(row["Desviación"])>thresh_rebal else "#1a7a4a") + "'>" +
                    f"{row['Desviación']*100:+.1f}%</td>"
                    f"<td style='color:{gc2}'>{gs}${row['Ganancia USD']:,.0f}</td>"
                    f"<td>{_badge(row['Alerta'])}</td></tr>"
                )
            st.markdown(
            ...
            st.info("Sin posiciones. Registra operaciones para ver tu portafolio.")

    with col_right:
        st.markdown("<div style='font-size:14px;font-weight:700;color:#1a3a5c;margin-bottom:10px;'>Distribución Actual vs Objetivo</div>",
                    unsafe_allow_html=True)
        if not alerts_df.empty:
            ...
            fig_pie = make_subplots(1, 2, specs=[[{"type":"pie"},{"type":"pie"}]],
                                    subplot_titles=["Actual","Objetivo"])
            fig_pie.add_trace(go.Pie(
                labels=alerts_df["Ticker"], values=alerts_df["Peso Actual"],
                hole=0.52, marker=dict(colors=[_tc3.get(t,"#888") for t in alerts_df["Ticker"]],
            ...
            fig_pie.add_trace(go.Pie(
                ...
                hole=0.52, marker=dict(colors=[_tc3.get(t,"#888") for t in target_wts],
                                       line=dict(color="rgba(255,255,255,0.7)",width=2)),
    ...
    # ── Evolución del portafolio modelo ──────────────────────────────────────
    if not model_history_df.empty:
        st.markdown("<div class='section-title'>📈 Evolución del Portafolio (Modelo)</div>",
                    unsafe_allow_html=True)
        fig_ev = go.Figure()
        ...
        fig_ev.add_trace(go.Bar(
            ...
            marker_color=["#1a7a4a" if r >= 0 else "#c0392b" for r in model_history_df["Retorno"]],
    ...
    # ── Admin: Frontera Eficiente + descarga PDF ──────────────────────────────
    if is_admin and model_result and model_result.get("frontier"):
        st.markdown("<div class='section-title'>🗺️ Frontera Eficiente (Modelo)</div>",
                    unsafe_allow_html=True)
        frontier = model_result["frontier"]
        f_vols = [pt["vol"] * 100 for pt in frontier]
        f_rets = [pt["ret"] * 100 for pt in frontier]
        fig_front = go.Figure()
        fig_front.add_trace(go.Scatter(
            x=f_vols, y=f_rets, mode="lines+markers",
            line=dict(color="#3b82f6", width=2.5),
        ...
        fig_front.add_trace(go.Scatter(
            x=[vt], y=[rt], mode="markers",
            marker=dict(size=14, color="#c9a227", symbol="star",
                       line=dict(width=2, color="#1a3a5c")),
            name=f"Tangencia (Sharpe={model_result.get('sharpe_tang',0):.2f})",
            hovertemplate=f"Tangencia<br>σ={vt:.1f}% · R={rt:.1f}%<extra></extra>"
        ))
        # Min var
        vm = model_result.get("vol_mv", 0) * 100
        rm = model_result.get("ret_mv", 0) * 100
        fig_front.add_trace(go.Scatter(
            x=[vm], y=[rm], mode="markers",
            marker=dict(size=12, color="#10b981", symbol="diamond"),
            name="Mínima Varianza",
            hovertemplate=f"Mín. Var<br>σ={vm:.1f}% · R={rm:.1f}%<extra></extra>"
        ))
        # CML
        rf_pct = model_result.get("rf", 0.0264) * 100
        cml_x = [0, max(f_vols) * 1.1]
        cml_y = [rf_pct, rf_pct + (rt - rf_pct) / vt * max(f_vols) * 1.1] if vt > 0 else [rf_pct, rf_pct]
        fig_front.add_trace(go.Scatter(
            x=cml_x, y=cml_y, mode="lines",
            line=dict(color="#ef4444", width=2, dash="dash"),
            name=f"CML (Rf={rf_pct:.2f}%)"
        ))
        fig_front.update_layout(
            height=380, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(232,240,248,0.2)",
        ...
        st.download_button(
            "📥 Descargar Reporte Completo PDF (Todos los Gráficos)",
            data=_pdf_bytes,
