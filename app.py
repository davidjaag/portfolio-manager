...
with st.sidebar:
    ...
    # ════════════════════════════════════════════════════════════════════════
    # ANÁLISIS MARKOWITZ COMPLETO — usa run_complete_analysis() del módulo
    # ════════════════════════════════════════════════════════════════════════
    # Lógica de carga:
    #   CON Sheet → carga precios históricos (Precios_<port>) → run_complete_analysis
    #   SIN Sheet → usa pesos MATLAB hardcodeados como fallback estático
    #               (sin datos sintéticos, sin optimizer — datos reales o nada)
    # ────────────────────────────────────────────────────────────────────────
    _mkz         = None
    _mkz_ok      = False
    _ret_source  = "MATLAB (estático)"
    _prices_df   = None      # DataFrame de precios (índice=fechas, cols=tickers)
    _sheets_warn = False     # bandera para mostrar advertencia de conexión

    if use_sheets and sheet_id:
        # 1️⃣ Intentar cargar precios históricos completos (retorna DataFrame de precios)
        _prices_df = load_prices_history_from_sheets(sheet_id, selected_port)

        if _prices_df is not None and len(_prices_df) >= 13:
            try:
                # 2️⃣ Correr el análisis completo de Markowitz
                _rf_port = port_data.get("rf", None)   # rf del portafolio; None → Yahoo Finance
                _mkz     = run_complete_analysis(_prices_df.dropna(axis=1), rf=_rf_port)

                # 3️⃣ Extraer pesos del portafolio de tangencia (filtra pesos < 0.1%)
                ...
                # Optimización fallida → usar pesos estáticos
                target_wts = port_data["target"]
                _mkz_ok    = False
                _ret_source = f"MATLAB (error análisis: {str(_mkz_err)[:60]})"
        else:
            # Hoja encontrada pero con datos insuficientes
            target_wts  = port_data["target"]
            _ret_source = "MATLAB (datos insuficientes en Sheet)"
    else:
        # Sin Sheet → fallback a pesos hardcodeados, mostrar advertencia
        target_wts   = port_data["target"]
        _sheets_warn = True   # se renderizará en main area

    # 4️⃣ Persistir en session_state para que todas las páginas accedan
    ...
    st.session_state["prices_df"]    = _prices_df      # precios históricos crudos
    st.session_state["sheets_warn"]  = _sheets_warn    # bandera para banner demo
