...
def core_model_run(prices_df: pd.DataFrame, rf: float = 0.0264,
    ...
    try:
        df = prices_df.copy()
        # Eliminar columna diagnóstico de fecha (no es un activo)
...
@st.cache_data(ttl=300)
def run_model(sid: str, pname: str, rf_val: float) -> dict:
    """
    Corre Markowitz para el portafolio indicado.
    Cacheado 5 min. Cada portafolio tiene su propia clave de caché.
    """
