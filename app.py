...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    ...
    with k3:
        _sr_val   = risk_metrics.get("sharpe", 0)
        _sr_obj   = port_data.get("matlab_sharpe", port_data.get("sharpe_obj", 1.0))
        _sr_ok    = _sr_val >= _sr_obj * 0.85          # dentro del 85% del objetivo
        _sr_color = "#1a7a4a" if _sr_ok else ("#d4820a" if _sr_val >= _sr_obj * 0.5 else "#c0392b")
        _sr_src   = "datos reales" if risk_metrics.get("source") == "modelo_real" else "ref. MATLAB"
        _sr_lbl, _ = sharpe_label(_sr_val)
        st.markdown(f"""<div class="kpi-card">
            <div class="kpi-label">Sharpe Ratio · {selected_port}</div>
            <div class="kpi-value" style="color:{_sr_color};">{_sr_val:.3f}</div>
            <div class="kpi-sub">{_sr_lbl} · Objetivo &gt; {_sr_obj:.2f} · <i>{_sr_src}</i></div>
        </div>""", unsafe_allow_html=True)
    with k4:
        _vol_val  = risk_metrics.get("vol_annual", 0)
        _vlo_, _vhi_ = port_data.get("volatilidad_range", (15, 25))
        _vol_ok   = _vol_val * 100 <= _vhi_
        _vc       = "#1a7a4a" if _vol_ok else "#c0392b"
        _ret_val  = risk_metrics.get("ret_annual", 0)
        _rlo_, _rhi_ = port_data.get("retorno_obj", (10, 30))
        st.markdown(f"""<div class="kpi-card">
            <div class="kpi-label">Volatilidad · {selected_port}</div>
            <div class="kpi-value" style="color:{_vc};">{_vol_val*100:.1f}%</div>
            <div class="kpi-sub">
                Objetivo: {_vlo_}%–{_vhi_}% · {"✅ En rango" if _vol_ok else "⚠ Fuera de rango"}<br>
                Retorno esperado: <b style='color:#1a3a5c;'>{_ret_val*100:.1f}%</b>
                (obj. {_rlo_}%–{_rhi_}%)
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    # ── Semáforo — umbrales 100% vinculados al portafolio activo ──────────────
    # Todas las comparaciones usan los rangos reales del portafolio seleccionado,
    # ya sean del modelo corrido o de los valores de referencia MATLAB.
    ...
    # Umbrales específicos de ESTE portafolio
    _vol_lo, _vol_hi = port_data.get("volatilidad_range", (15, 25))
    _ret_lo, _ret_hi = port_data.get("retorno_obj",       (10, 30))
    _sr_obj          = port_data.get("matlab_sharpe", port_data.get("sharpe_obj", 1.0))
    _vth             = _vol_hi / 100   # umbral vol en decimal
    # VaR objetivo: percentil 5 anual de MATLAB / 12 meses
    _var_ref         = abs(port_data.get("matlab_p5", -0.05)) / 12

    # Etiquetas contextuales por perfil
    _profile = port_data.get("profile", "moderado")
    _sr_good_lbl  = "Excelente para tu perfil"      if _sr >= _sr_obj else "Dentro del objetivo"
    _sr_bad_lbl   = f"Por debajo del objetivo ({_sr_obj:.2f}) — revisa con asesor"
    _vol_good_lbl = f"Volatilidad dentro del rango {_vol_lo}%–{_vol_hi}%"
    _vol_bad_lbl  = f"Alta para {selected_port}: esperado ≤ {_vol_hi:.0f}%"
    _dd_thresh    = -0.10 if _profile == "conservador" else (-0.20 if _profile in ("moderado","balanceado") else -0.35)
    _dd_good_lbl  = "Caída controlada para tu perfil"
    _dd_bad_lbl   = "Caída por encima de lo esperado"
    _var_good_lbl = f"Riesgo mensual aceptable para {selected_port}"
    _var_bad_lbl  = "Riesgo mensual elevado para tu perfil"

    sem_cols = st.columns(4)
    _sem_data = [
        # col, icon_ok, icon_bad, label, value_str, txt_ok, txt_bad, is_good
        (sem_cols[0], "🟢", "🔴",
         f"Eficiencia · {selected_port}",
         f"{_sr:.3f}",
         _sr_good_lbl, _sr_bad_lbl,
         _sr >= _sr_obj * 0.85),

        (sem_cols[1], "🟢", "🟡",
         f"Volatilidad · {selected_port}",
         f"{_vol*100:.1f}%",
         _vol_good_lbl, _vol_bad_lbl,
         _vol <= _vth),

        (sem_cols[2], "🟢", "🔴",
         f"Caída Máxima · {selected_port}",
         f"{_dd*100:.1f}%",
         _dd_good_lbl, _dd_bad_lbl,
         _dd >= _dd_thresh),

        (sem_cols[3], "🟢", "🟡",
         f"VaR 95% mensual · {selected_port}",
         f"{abs(_var)*100:.2f}%",
         _var_good_lbl, _var_bad_lbl,
         abs(_var) <= _var_ref * 1.2),
    ]

    for _col, _ico_g, _ico_b, _lbl, _val_s, _txt_g, _txt_b, _is_good in _sem_data:
        _ico   = _ico_g if _is_good else _ico_b
        _color = "#1a7a4a" if _is_good else "#c0392b"
        _txt   = _txt_g  if _is_good else _txt_b
        with _col:
            st.markdown(f"""
            <div style='background:white;border-radius:12px;padding:14px;text-align:center;
                        box-shadow:0 2px 10px rgba(13,27,42,0.07);border-top:4px solid {_color};'>
                <div style='font-size:20px;margin-bottom:3px;'>{_ico}</div>
                <div style='font-size:9px;text-transform:uppercase;letter-spacing:.08em;
                            color:#6b7c93;margin-bottom:3px;'>{_lbl}</div>
                <div style='font-family:"DM Serif Display",serif;font-size:22px;color:{_color};'>{_val_s}</div>
                <div style='font-size:10px;color:{_color};font-weight:600;margin-top:3px;
                            line-height:1.4;'>{_txt}</div>
            </div>""", unsafe_allow_html=True)
