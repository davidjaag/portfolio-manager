...
# ══════════════════════════════════════════════════════════════════════════
def core_model_run(prices_df: pd.DataFrame, rf: float = 0.0264,
                   max_w: float = 0.30) -> dict:
    from scipy.optimize import minimize

    if prices_df is None or prices_df.empty or len(prices_df) < 13:
        return {"error": "insuficientes_datos", "n_rows": len(prices_df) if prices_df is not None else 0}

    try:
        df = prices_df.copy()
        # Eliminar filas con NaN (igual que MATLAB: filas_validas = all(~isnan(Precios),2))
        df = df.dropna()

        # ── Identificar SPX benchmark ─────────────────────────────────────────
        # IMPORTANTE: igual que MATLAB, SPX participa en la optimización como
        # activo más (puede recibir peso). Se usa ADICIONALMENTE como benchmark
        # para calcular betas en la SML.
        # MATLAB no excluye SPX del portafolio — lo trata como un activo más.
        spx_col = next((c for c in df.columns if "SPX" in str(c).upper()), None)

        # Todos los activos incluyendo SPX entran al optimizador
        asset_cols = list(df.columns)
        n = len(asset_cols)
        if n < 2:
            return {"error": "menos_de_2_activos", "activos": asset_cols}

        # ── Rendimientos logarítmicos (diff(log(P)) igual que MATLAB) ──────
        P = df[asset_cols].values.astype(float)
        R = np.diff(np.log(P), axis=0)        # shape: (T-1, n)
        T = R.shape[0]

        mu  = R.mean(axis=0) * 12             # retorno anual esperado
        cov = np.cov(R.T, ddof=1) * 12        # covarianza anual

        # ── Portafolio Tangencia: max Sharpe con límite max_w por activo ────
        def neg_sr(w):
            r = float(w @ mu)
        ...
        res_t = minimize(neg_sr, w0, method="SLSQP", bounds=bnd, constraints=cons,
                         options={"ftol": 1e-12, "maxiter": 5000})
        wt = np.clip(res_t.x, 0, max_w); wt /= wt.sum()
        ret_t = float(wt @ mu)
        vol_t = float(np.sqrt(wt @ cov @ wt))
        sr_t  = (ret_t - rf) / vol_t if vol_t > 1e-10 else 0.0

        # ── Mínima Varianza ──────────────────────────────────────────────────
        def port_var(w): return float(w @ cov @ w)
        res_mv = minimize(port_var, w0, method="SLSQP", bounds=bnd, constraints=cons,
                          options={"ftol": 1e-12, "maxiter": 5000})
        wv = np.clip(res_mv.x, 0, max_w); wv /= wv.sum()
        ret_v = float(wv @ mu)
        vol_v = float(np.sqrt(wv @ cov @ wv))

        # ── Frontera Eficiente (20 puntos igual que MATLAB numPorts=20) ──────
        frontier = []
        for tgt in np.linspace(ret_v, float(np.max(mu)) * 0.99, 20):
            c2 = cons + [{"type": "eq", "fun": lambda w, r=tgt: float(w @ mu) - r}]
            r2 = minimize(port_var, w0, method="SLSQP", bounds=bnd, constraints=c2,
                          options={"ftol": 1e-10, "maxiter": 500})
            if r2.success:
                wf = np.clip(r2.x, 0, max_w); wf /= wf.sum()
                frontier.append({
                    "vol": float(np.sqrt(wf @ cov @ wf)),
                    "ret": float(wf @ mu),
                    "weights": {asset_cols[i]: float(wf[i]) for i in range(n)}
                })

        # ── SML / Betas usando SPX como benchmark ───────────────────────────
        # SPX está en asset_cols (participa en portafolio), pero también
        # se usa como Rm para calcular betas de todos los activos vs mercado.
        betas, alphas = {}, {}
        if spx_col is not None and spx_col in asset_cols:
            spx_idx = asset_cols.index(spx_col)
            Rm      = R[:, spx_idx]          # rendimientos del benchmark
            Rm_ann  = float(Rm.mean() * 12)
            MRP     = Rm_ann - rf
            vm      = float(np.var(Rm, ddof=1))
            for i, t in enumerate(asset_cols):
                if t == spx_col:
                    betas[t] = 1.0           # beta del mercado vs sí mismo = 1
                    alphas[t] = 0.0
                else:
                    cov_im    = float(np.cov(R[:, i], Rm)[0, 1])
                    beta_i    = cov_im / vm if vm > 0 else 1.0
                    betas[t]  = beta_i
                    alphas[t] = mu[i] - (rf + beta_i * MRP)
        else:
            for t in asset_cols:
        ...
        # ── Monte Carlo 10 000 sims (igual que MATLAB) ───────────────────────
        np.random.seed(42)
        NS = 10000
        sims = np.zeros(NS)
        mu_m = mu / 12; cov_m = cov / 12
        for k in range(NS):
            sr = np.random.multivariate_normal(mu_m, cov_m, 12)
            sims[k] = float(np.prod(1 + sr @ wt) - 1)

        # ── VaR / CVaR individuales ──────────────────────────────────────────
        var_a, cvar_a = {}, {}
        acc = np.zeros((n, NS))
        for k in range(NS):
            sr = np.random.multivariate_normal(mu_m, cov_m, 12)
            for i in range(n):
                acc[i, k] = float(np.prod(1 + sr[:, i]) - 1)
        for i, t in enumerate(asset_cols):
            s = np.sort(acc[i])
            ix = max(0, int(0.05 * NS) - 1)
            var_a[t]  = -float(s[ix])
            cvar_a[t] = -float(s[:ix+1].mean())

        # ── Intervalos por horizonte ─────────────────────────────────────────
        hz = {}
        for yr in [1, 3, 5, 10, 20]:
            m = yr * 12
            s = np.zeros(NS)
            for k in range(NS):
                sr = np.random.multivariate_normal(mu_m, cov_m, m)
                s[k] = float((np.prod(1 + sr @ wt) - 1) * 100)
            hz[yr] = {"p5": float(np.percentile(s, 5)),
                      "mean": float(s.mean()),
                      "p95": float(np.percentile(s, 95))}

        # ── Histórico: retornos del portafolio tangencia ─────────────────────
        port_r = R @ wt
        ...
        return {
            ...
            "max_w":       max_w,
            # Tangencia
            "w_tang":      {asset_cols[i]: float(wt[i]) for i in range(n)},
            "ret_tang":    ret_t,
            "vol_tang":    vol_t,
            "sharpe_tang": sr_t,
            # Mín Varianza
            "w_mv":        {asset_cols[i]: float(wv[i]) for i in range(n)},
            "ret_mv":      ret_v,
            "vol_mv":      vol_v,
            # Frontera
            "frontier":    frontier,
            # Matrices
            "mu_ann":      {asset_cols[i]: float(mu[i]) for i in range(n)},
            "cov_ann":     cov,
            "corr":        corr,
            "sd_ann":      {asset_cols[i]: float(sd[i]) for i in range(n)},
            # SML
            "betas":       betas,
            "alphas":      alphas,
            # Riesgo histórico
            ...
            # Monte Carlo
            "mc_sims":     sims.tolist(),
            "mc_p5":       float(np.percentile(sims, 5)),
...
def load_prices(sheet_id: str, port_name: str) -> pd.DataFrame:
    ...
    Devuelve DataFrame de precios (filas=fechas, cols=tickers).
    ...
    try:
        ...
        for row in data[1:]:
            ...
            for j, col in enumerate(headers):
                if col.lower() in ("fecha","date",""): continue
                tkr = B2T.get(col) or col.strip().upper()
                try:
                    v = float(str(row[j]).replace(",",".").replace(" ",""))
                    if v > 0: vals[tkr] = v
                except Exception: pass
            if len(vals) >= 2: rows.append(vals)
        if len(rows) < 13: return pd.DataFrame()
...
if selected:
    ...
    # ── Correr modelo solo con los datos de este portafolio
    @st.cache_data(ttl=3600)
    def run_model(sid, pname, rf_val):
        df = load_prices(sid, pname)
        if df.empty: return {}
        return core_model_run(df, rf=rf_val, max_w=0.30)

    model = {}
    if use_sheets and sheet_id and not prices_hist.empty:
        model = run_model(sheet_id, selected, port_cfg.get("rf", 0.0264))
    ...
    if model and not model.get("error") and model.get("w_tang"):
        target_wts = {t:w for t,w in model["w_tang"].items() if w >= 0.001}
        wt_source  = f"Modelo Markowitz · {model['n_obs']} meses"
    else:
        target_wts = seed_wts
        wt_source  = "Pesos semilla (conecta Sheet para el modelo)"

    # ── Precios actuales (YF > ops > semilla)
    _yf = get_quotes(list(target_wts.keys()) +
                     (ops_df["Ticker"].unique().tolist() if not ops_df.empty else []))
    yf_prices = {t: d.get("price") or 0 for t,d in _yf.items() if d.get("price")}
    ops_prices = {}
    if not ops_df.empty and "Precio_USD" in ops_df.columns:
        ops_prices = ops_df.sort_values("Fecha").groupby("Ticker")["Precio_USD"].last().to_dict()
    seed_prices = {t: 100.0 for t in target_wts}  # fallback si todo falla
    prices = {**seed_prices, **ops_prices, **yf_prices}
    px_source = "Yahoo Finance" if yf_prices else ("Operaciones" if ops_prices else "Semilla")
    ...
    # ── Métricas del modelo (nunca vacías)
    if model and not model.get("error"):
        rm = {
            "sharpe": model["sharpe_tang"], "vol":   model["vol_tang"],
            "ret":    model["ret_tang"],     "dd":    model["max_dd"],
            "var95":  model["var_95"],        "n":    model["n_obs"],
            "source": "modelo",
        ...
        # Semilla del config
        rm = {
            "sharpe": port_cfg.get("matlab_sharpe",0),
            "vol":    port_cfg.get("matlab_vol",   0),
            "ret":    port_cfg.get("matlab_ret",   0),
            "dd":     port_cfg.get("matlab_p5",    0),
            "var95":  port_cfg.get("matlab_p5",    0) / 12 if port_cfg.get("matlab_p5") else 0,
            "n":      0,
            "source": "semilla",
    ...
    # Esto permite verificar exactamente qué hoja se está leyendo y qué tickers tiene
    if sheet_id:
        _cols = list(prices_hist.columns) if not prices_hist.empty else []
        _rows = len(prices_hist)
        _spx  = next((c for c in _cols if "SPX" in c.upper()), None)
        _act  = [c for c in _cols if _spx and c != _spx or not _spx]
        if prices_hist.empty:
            _diag_cls = "diag-err"
            _diag_msg = (f"❌ No se encontró la hoja <b>Precios_{selected}</b> en el Sheet "
                        f"<code>{sheet_id[:20]}…</code> — verifica que la hoja exista.")
        elif _rows < 13:
            _diag_cls = "diag-warn"
            _diag_msg = (f"⚠ Hoja <b>Precios_{selected}</b> encontrada pero solo tiene "
                        f"<b>{_rows} filas</b> (mínimo 13). El modelo necesita más historia.")
        else:
            _model_err = model.get("error") if model else "sin_sheet"
            if _model_err:
                _diag_cls = "diag-warn"
                _diag_msg = (f"⚠ Datos cargados pero modelo falló: <b>{_model_err}</b>")
            else:
                _diag_cls = "diag-ok"
                _diag_msg = (f"✅ Sheet conectado · Hoja: <b>Precios_{selected}</b> · "
                            f"<b>{_rows} filas</b> · Activos en Excel: <b>{', '.join(_cols)}</b>")
        st.markdown(f"<div class='diag {_diag_cls}'>{_diag_msg}</div>",
                   unsafe_allow_html=True)
    else:
        st.markdown(f"<div class='diag diag-warn'>"
                   f"⚠ Sheet no configurado. Pesos semilla activos para <b>{selected}</b>.</div>",
                   unsafe_allow_html=True)
    ...
    if is_a:
        st.markdown(
            f"<div class='dbg'>MODELO [{selected}] Activos: ",
            f"Fuente: <b>{rm['source']}</b> · "
            f"Activos modelo: <b>{rm['activos']}</b> · "
            f"Sharpe: <b>{rm['sharpe']:.4f}</b> · "
            f"Vol: <b>{rm['vol']*100:.2f}%</b> · "
            f"Ret: <b>{rm['ret']*100:.2f}%</b> · "
            f"n_meses: <b>{rm['n']}</b> · "
            f"{'  |  Posiciones: ' + str(len(_op_tickers)) + ' tickers' if _op_tickers else '  |  Sin operaciones'}</div>",
            unsafe_allow_html=True)

    # ── PANEL DE VALIDACIÓN DE ACTIVOS ────────────────────────────────────────
    # Compara los activos del Excel vs los pesos semilla configurados.
    # Si difieren → el modelo correrá con los del Excel (que puede ser correcto o incorrecto).
    _excel_activos = [c for c in prices_hist.columns
                      if not prices_hist.empty and "SPX" not in c.upper()
                     ] if not prices_hist.empty else []
    _seed_activos  = [t for t in port_cfg.get("target",{}).keys()
                      if "SPX" not in t.upper()]
    if _excel_activos and _seed_activos:
        _extra   = [t for t in _excel_activos if t not in _seed_activos]
        _missing = [t for t in _seed_activos  if t not in _excel_activos]
        if _extra or _missing:
            _msg_parts = []
            if _extra:
                _msg_parts.append(
                    f"Tickers en Excel <b>no esperados</b>: "
                    f"<b style='color:#ef4444;'>{', '.join(_extra)}</b>")
            if _missing:
                _msg_parts.append(
                    f"Tickers <b>ausentes</b> en Excel: "
                    f"<b style='color:#d97706;'>{', '.join(_missing)}</b>")
            st.markdown(
                f"<div class='diag diag-warn'>"
                f"⚠ <b>Discrepancia de activos en Precios_{selected}:</b> "
                f"{' · '.join(_msg_parts)}<br>"
                f"<span style='font-size:11px;color:#6b7280;'>"
                f"El modelo corre con los activos del Excel. "
                f"Si el Sharpe no coincide con MATLAB, revisa que la hoja "
                f"<b>Precios_{selected}</b> tenga exactamente los mismos tickers "
                f"que usó MATLAB: "
                f"<b>{', '.join(_seed_activos)}</b> + SPX.</span></div>",
                unsafe_allow_html=True)
...
if page in ("📊 Dashboard","📊 Mi Portafolio") and selected:
    ...
    sr_obj = port_cfg.get("matlab_sharpe", port_cfg.get("sharpe_obj",1.0))
    vlo,vhi = port_cfg.get("volatilidad_range",(4,30))
    rlo,rhi = port_cfg.get("retorno_obj",(5,40))
    ...
    with c3:
        sc = "#059669" if rm['sharpe']>=sr_obj else ("#d97706" if rm['sharpe']>=sr_obj*0.5 else "#ef4444")
        st.markdown(f"""<div class='card'>
            <div class='kpi-lbl'>Sharpe · {selected}</div>
            <div class='kpi-val' style='color:{sc};'>{rm['sharpe']:.3f}</div>
            <div class='kpi-sub'>Objetivo: &gt;{sr_obj:.2f} · {rm['source']}</div>
        </div>""", unsafe_allow_html=True)
    with c4:
        vc = "#059669" if rm['vol']*100<=vhi else "#ef4444"
        st.markdown(f"""<div class='card'>
            <div class='kpi-lbl'>Volatilidad · {selected}</div>
            <div class='kpi-val' style='color:{vc};'>{rm['vol']*100:.1f}%</div>
            <div class='kpi-sub'>Objetivo: {vlo}%–{vhi}% · Ret. modelo: {rm['ret']*100:.2f}%</div>
        </div>""", unsafe_allow_html=True)

    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

    # ── Semáforos ─────────────────────────────────────────────────────────
    profile  = port_cfg.get("profile","moderado")
    dd_thr   = -0.08 if profile=="conservador" else (-0.18 if profile in ("moderado","balanceado") else -0.38)
    var_ref  = abs(port_cfg.get("matlab_p5",-0.07)) / 12

    s1,s2,s3,s4 = st.columns(4)
    def sem(col, ico_ok, ico_bad, lbl, val, txt_ok, txt_bad, good, color_ok="#059669", color_bad="#ef4444"):
        ico   = ico_ok if good else ico_bad
        color = color_ok if good else color_bad
        txt   = txt_ok if good else txt_bad
        with col:
    ...
    sem(s1,"🟢","🔴",f"Sharpe · {selected}",f"{rm['sharpe']:.3f}",
        f"Cumple obj ({sr_obj:.2f})",f"Bajo obj ({sr_obj:.2f})",
        rm['sharpe']>=sr_obj*0.85)
    sem(s2,"🟢","🟡",f"Volatilidad · {selected}",f"{rm['vol']*100:.1f}%",
        f"Dentro {vlo}%–{vhi}%",f"Alta — obj ≤{vhi}%",
        rm['vol']*100<=vhi, color_bad="#d97706")
    sem(s3,"🟢","🔴",f"Caída Máx · {selected}",f"{rm['dd']*100:.1f}%",
        "Controlada para perfil",f"Alta (ref {dd_thr*100:.0f}%)",
        rm['dd']>=dd_thr)
    sem(s4,"🟢","🟡",f"VaR 95% · {selected}",f"{abs(rm['var95'])*100:.2f}%",
        "Riesgo aceptable","Elevado para perfil",
        abs(rm['var95'])<=var_ref*1.2, color_bad="#d97706")
...
# ── ADMIN ──────────────────────────────────────────────────────────────────
    ...
    with st.expander("➕ Crear Nuevo Portafolio", expanded=not bool(get_all_ports())):
        st.markdown("""<div class='diag diag-ok'>
            <b>Cómo funciona:</b> Crea el portafolio aquí → El modelo lee automáticamente
            <b>Precios_NOMBRE</b> y <b>Ops_NOMBRE</b> del Sheet.
            Los activos los define tu Excel — no el código.
            Agrega la hoja con los tickers correctos y el modelo corre automáticamente.
        </div>""", unsafe_allow_html=True)
        a1,a2,a3=st.columns(3)
        with a1:
            np_name=st.text_input("Nombre (sin espacios)",placeholder="Conservador",key="npn")
            np_desc=st.text_input("Descripción",placeholder="Perfil Conservador",key="npd")
            np_color=st.color_picker("Color",value="#3b82f6",key="npc")
        with a2:
            np_profile=st.selectbox("Perfil de riesgo",
                ["conservador","moderado","balanceado","agresivo"],key="npp")
            np_rf=st.number_input("Tasa libre de riesgo (%)",value=2.64,step=0.01,format="%.2f")/100
            np_maxw=st.number_input("Límite máximo por activo (%)",value=30.0,step=1.0,format="%.0f")/100
        with a3:
            np_vlo=st.number_input("Volatilidad mín objetivo (%)",value=4.0,step=0.5)/100
            np_vhi=st.number_input("Volatilidad máx objetivo (%)",value=8.0,step=0.5)/100
            np_rlo=st.number_input("Retorno mín objetivo (%)",value=3.0,step=0.5)/100
            np_rhi=st.number_input("Retorno máx objetivo (%)",value=8.0,step=0.5)/100
        np_matlab=st.number_input("Sharpe objetivo (referencia MATLAB)",value=0.13,step=0.01,format="%.3f")

        st.markdown("<div style='margin-top:8px;font-size:11px;color:#6b7280;'>"
                   "ℹ Los pesos objetivo los calculará el modelo automáticamente desde "
                   f"la hoja <b>Precios_{{nombre}}</b> del Sheet.</div>", unsafe_allow_html=True)

        if st.button("➕ Crear Portafolio", key="btn_create", type="primary"):
            n=np_name.strip().replace(" ","")
            if not n: st.error("Nombre requerido")
            elif n in get_all_ports(): st.error(f"Ya existe '{n}'")
            else:
                st.session_state.custom_portfolios[n] = {
                    ...
                    "max_w":       np_maxw,
                    "profile":     np_profile,
                    "target":      {},   # vacío: el modelo lo llena desde el Excel
                    "matlab_sharpe": np_matlab,
                    "matlab_ret":  0.0, "matlab_vol":  0.0,
                    "matlab_p5":   0.0, "matlab_p95":  0.0,
                    "volatilidad_range": (np_vlo*100, np_vhi*100),
                    "retorno_obj":       (np_rlo*100, np_rhi*100),
                    "sharpe_obj":  np_matlab,
                    "icon":        {"conservador":"🛡️","moderado":"⚖️",
                                    "balanceado":"📊","agresivo":"🚀"}.get(np_profile,"📁"),
                    "bg":          {"conservador":"linear-gradient(135deg,#ecfdf5,#d1fae5)",
                                    "moderado":"linear-gradient(135deg,#fffbeb,#fef3c7)",
                                    "balanceado":"linear-gradient(135deg,#eff6ff,#dbeafe)",
                                    "agresivo":"linear-gradient(135deg,#fef2f2,#fee2e2)"
                                   }.get(np_profile,"linear-gradient(135deg,#f9fafb,#f3f4f6)"),
                    "border":      np_color,
                    "horizonte":   {"conservador":"1–4 años","moderado":"3–7 años",
                                    "balanceado":"5–10 años","agresivo":"5–10 años"}.get(np_profile,""),
                    "tolerancia":  {"conservador":"Muy Baja","moderado":"Media",
                                    "balanceado":"Media-Alta","agresivo":"Alta"}.get(np_profile,"Media"),
                    "tolerancia_pct":{"conservador":15,"moderado":45,
                                      "balanceado":55,"agresivo":80}.get(np_profile,50),
                    "horizonte_years":{"conservador":3,"moderado":5,
                                       "balanceado":7,"agresivo":8}.get(np_profile,5),
                }
                st.success(f"✅ Portafolio '{n}' creado. "
                          f"Asegúrate de tener la hoja Precios_{n} en tu Sheet.")
                st.rerun()
    ...
    if all_p:
        ...
        for pn,pd2 in list(all_p.items()):
            ...
            with c1:
                _mod=run_model(sheet_id,pn,pd2.get("rf",0.0264)) if use_sheets and sheet_id else {}
                ...
                st.markdown(
                    ...
                    f"<div style='font-size:12px;color:#6b7280;'>{pd2.get('description','')}"
                    f" · Perfil: {pd2.get('profile','')}"
                    f" · Sharpe: <b>{_sr_s}</b> · Retorno: <b>{_rt_s}</b></div>"
                    f"<div style='font-size:10px;color:#9ca3af;margin-top:4px;'>"
                    f"Sheet: Precios_{pn} + Ops_{pn}</div></div>",
                    unsafe_allow_html=True)
            with c2:
