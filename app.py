...
USERS = {  # USERS_START
    "david": {
        ...
        "avatar":     "DJ",
        "color":      "#1a3a5c",
        "sheet_id":   "",           # ← pegar Sheet ID real aquí
    ...
    try:
        ...
        for tab in [f"Precios_{portfolio_name}", "Precios"]:
            try:
                ...
                # Calcular retornos: r_t = P_t/P_{t-1} - 1
        ...
        for i, row in price_df.iterrows():
            ...
            for tkr in price_df.columns:
                ...
                if qty > 0 and price_now > 0:
                    portfolio_value += qty * price_now * tc
...
with st.sidebar:
    ...
    with st.expander("⚙ Configuración Google Sheets", expanded=False):
        st.markdown("<small style='color:#aab;'>Ingresa tu Sheet ID para conectar datos reales</small>", unsafe_allow_html=True)
        ...
        if sheet_id:
            ...
            if client:
                ...
                st.warning("Configura las credenciales en secrets.toml")
        else:
    ...
    if use_sheets and sheet_id:
        ...
        if _ret_df_raw is not None and len(_ret_df_raw) >= 12:
            try:
                ...
                target_wts = {t: w for t, w in _wts_raw.items() if w >= 0.005}
                if not target_wts:
...
risk_metrics = calc_risk_metrics(history)
...
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    ...
    for uname, udata in USERS.items():
        ...
        <div style='background:#1a3a5c;color:white;border-radius:12px 12px 0 0;
          ...
          <div>
            ...
            <div style='font-size:11px;color:#c9d6e3;'>
              {udata.get("role","investor").upper()} · {len(uports)} portafolio(s) asignado(s)
            </div>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        for pname in uports:
            ...
            # Load ops for this user/portfolio
            ...
            with c1:
                ...
                <div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>
                    📁 {pname}
                  </div>
                  <div style='font-size:20px;font-weight:800;color:#1a3a5c;'>
                    ${total_mxn_u:,.0f}
                  </div>
            ...
            with c3:
                ...
                <div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
                  ...
                  <div style='font-size:11px;color:#6b7c93;'>
                    Ret {ret_u*100:.1f}% · Vol {vol_u*100:.1f}%
                  </div>
                </div>""", unsafe_allow_html=True)
            with c4:
                st.markdown(f"""
                <div style='background:white;border:1px solid #e8eef5;
                     border-radius:0 0 12px 0;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>
                    🎯 Pesos objetivo
                  </div>
