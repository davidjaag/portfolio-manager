...
def core_model_run(prices_df: pd.DataFrame, rf: float = 0.0264,
    ...
    try:
        df = prices_df.copy()
        # Eliminar filas con NaN (igual que MATLAB)
        df = df.dropna()

        # Separar SPX (benchmark) de los activos del portafolio
        spx_col = next((c for c in df.columns if "SPX" in str(c).upper()), None)
        asset_cols = [c for c in df.columns if c != spx_col]
        n = len(asset_cols)
        if n < 2:
        ...
        if spx_col is not None:
            Rm = np.diff(np.log(df[spx_col].values.astype(float)))
            L  = min(len(R), len(Rm))
            Rm, Ra = Rm[-L:], R[-L:]
...
if page in ("📊 Dashboard","📊 Mi Portafolio") and selected:
    ...
    with pc2:
        ...
        if not alerts_df.empty and alerts_df["Peso Actual"].sum() > 0:
            pal=["#1a3a5c","#c9a227","#059669","#ef4444","#5b4fcf","#0e7490","#b45309","#6d28d9"]
            all_t=list(dict.fromkeys(list(alerts_df["Ticker"])+list(target_wts.keys())))
            tc3={t:pal[i%len(pal)] for i,t in enumerate(all_t)}
            ...
            fig.add_trace(go.Pie(
                ...
                marker=dict(colors=[tc3.get(t,"#888") for t in alerts_df["Ticker"]],
                           line=dict(color="white",width=2)),
                showlegend=True, hovertemplate="<b>%{label}</b><br>%{percent}<extra></extra>"
            ),1,1)
            fig.add_trace(go.Pie(
                ...
                marker=dict(colors=[tc3.get(t,"#888") for t in target_wts],
                           line=dict(color="white",width=2)),
                showlegend=False, hovertemplate="<b>%{label}</b><br>%{percent}<extra></extra>"
            ),1,2)
