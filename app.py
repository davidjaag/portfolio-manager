...
def calc_risk_metrics_bloomberg(log_ret_df, weights, rf=0.0264):
    try:
        ...
        roll_v  = None
        if len(pr) >= 6:
            roll_v = float(pd.Series(pr).rolling(6).std().iloc[-1] * np.sqrt(12))
...
with st.sidebar:
    ...
    # use_sheets = True siempre que haya sheet_id — gspread maneja errores internamente
    ...
    if use_sheets:
        ...
        if client:
            ...
            st.sidebar.warning("⚠ Credenciales no configuradas en secrets.toml")
    ...
    if use_sheets and sheet_id:
        ...
        if _ret_df_raw is not None and len(_ret_df_raw) >= 12:
            try:
                ...
                target_wts = port_data["target"]
                st.sidebar.error(f"Optimizer error: {_e}")
        else:
            target_wts = port_data["target"]
    else:
        target_wts = port_data["target"]
    ...
    if _is_admin:
        _pages = ["📊 Dashboard", "📋 Operaciones", "⚡ Análisis de Riesgo", "👥 Usuarios"]
    else:
