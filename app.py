...
with st.sidebar:
    ...
    # Indicador Google Sheets — solo si hay error (no mostrar cuando funciona)
    if not use_sheets and not sheet_id:
        st.caption("📊 Datos demo — sin Sheet conectado")
    elif not use_sheets and sheet_id:
    ...
    if _is_admin_sb:
        ...
        thresh_rebal  = 0.05
        st.markdown(
            f"<div style='background:linear-gradient(135deg,#1a3a5c,#0d1b2a);border-radius:10px;"
            f"padding:12px 16px;margin-bottom:4px;'>"
            f"<div style='color:#c9a227;font-size:10px;font-weight:700;letter-spacing:.1em;"
            f"text-transform:uppercase;margin-bottom:2px;'>USD / MXN</div>"
            f"<div style='display:flex;align-items:baseline;gap:6px;'>"
            f"<span style='font-size:26px;font-weight:800;color:#fff;'>{_tc_yf:.2f}</span>"
            f"<span style='font-size:10px;color:rgba(255,255,255,0.45);'>tiempo real</span>"
            f"</div></div>",
            unsafe_allow_html=True
        )
