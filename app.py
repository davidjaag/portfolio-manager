...
if selected:
    ...
    if is_a:
        ...
        # ── Diagnóstico de datos: primeros y últimos precios del Sheet ─────
        # Esto permite comparar exactamente qué leyó la app vs tu Excel local
        _pcols = [c for c in prices_hist.columns if not c.startswith("__")]
        _diag_rows = ""
        if not prices_hist.empty and len(_pcols) >= 1:
            ...
            _diag_rows  = (f"<br><span style='color:#86efac;'>1ª fila [{_fecha_primera}]: {_first_vals}</span>"
