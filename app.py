...
with st.sidebar:
    ...
    # Sheet: usa el propio si tiene, si no hereda el del admin (Sheet compartido)
    ...
    # Global params — TC desde Yahoo Finance (editable)
    _tc_yf = 20.50
    if YF_AVAILABLE:
        try:
    ...
    _is_admin_sidebar = user_data.get("role") == "admin"
...
# Enriquecer con el último Precio_USD de cada ticker en las ops del usuario
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    ...
    if not _is_admin:
        ...
        if not YF_AVAILABLE:
            st.info("📡 Cargando datos de Yahoo Finance...")
        else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("inv_op_form", clear_on_submit=True):
        ...
        with c2:
            # Universo completo del portafolio (modelo + tickers reales en ops)
            _universe = port_data.get("universe", list(target_wts.keys()))
            _op_tickers_existing = ops_df["Ticker"].unique().tolist() if not ops_df.empty else []
            port_tickers_list = _universe + [t for t in _op_tickers_existing if t not in _universe]
...
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    ...
    for uname, udata in USERS.items():
        ...
        for pname in uports:
            ...
            sharpe_u=u_rm.get("sharpe",0);vol_u=u_rm.get("vol_annual",0);ret_u=u_rm.get("ret_annual",0)
