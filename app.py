...
with st.sidebar:
    ...
    # Google Sheets — se conecta automáticamente con el sheet_id del usuario
    ...
    with st.expander("⚙ Configuración Google Sheets", expanded=False):
        if use_sheets:
            ...
            st.markdown(f"<small style='color:#aab;'>Sheet ID: <code>{sheet_id}</code></small>", unsafe_allow_html=True)
        else:
            st.info("Sin Sheet ID configurado — usando datos de demostración")
