...
def calc_alerts(positions_df, target_weights, tc, monthly_contrib,
    ...
    # Siempre calcular Peso Objetivo (incluyendo tickers sin modelo → 0)
    df["Peso Objetivo"] = df["Ticker"].map(target_weights).fillna(0.0)
    if total_mxn == 0:
        # Sin inversión aún: mostrar qué comprar según el modelo
        df["Peso Actual"]         = 0.0
...
with st.sidebar:
    ...
    tc_actual = st.number_input("TC MXN/USD", value=_tc_yf, step=0.01, format="%.2f",
        help=f"Sugerido por Yahoo Finance: {_tc_yf:.2f} · editable para registros de otra fecha")
...
if use_sheets and sheet_id:
    ...
    if ops_raw is not None and not ops_raw.empty:
        if "Usuario" in ops_raw.columns:
            _f = ops_raw[ops_raw["Usuario"].astype(str).str.lower() == current_user.lower()].copy()
...
# Enriquecer prices con el último precio registrado en las operaciones
# Esto garantiza que tickers con ops pero sin Sheet de precios muestren valor real
if not ops_df.empty and "Precio_USD" in ops_df.columns and "Ticker" in ops_df.columns:
    _ops_prices = (
        ops_df.sort_values("Fecha")
        .groupby("Ticker")["Precio_USD"]
        .last()
        .to_dict()
    )
    if prices is None:
        prices = _ops_prices
        prices_source = "Último precio registrado en operaciones"
    else:
        # Solo rellenar tickers que faltan en el Sheet
        for _t, _p in _ops_prices.items():
            if _t not in prices or prices[_t] == 0:
                prices[_t] = _p
...
# Incluir tickers del modelo + tickers reales de ops (sin dejar ninguno invisible)
...
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    st.markdown("<div class='section-title'>🔭 QuickView — Vista de todos los portafolios</div>", unsafe_allow_html=True)
    ...
    for uname, udata in USERS.items():
        ...
        for pname in uports:
                ...
                try:
                    ...
                    if _raw is not None and not _raw.empty:
                        if "Usuario" in _raw.columns:
                            _f=_raw[_raw["Usuario"].astype(str).str.lower()==uname.lower()].copy()
                            u_ops=_f if not _f.empty else _raw
                        else: u_ops=_raw
                except: pass
            if u_ops.empty: u_ops=get_demo_operations(pname)
            # precios: último precio de las ops
            _u_prices={}
            ...
            with c2: st.markdown(f"<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;border-radius:8px;'><div style='font-size:11px;color:#6b7c93;font-weight:700;'>📋 Operaciones</div><div style='font-size:20px;font-weight:800;color:#1a3a5c;'>{n_ops}</div><div style='font-size:11px;color:#6b7c93;'>registradas</div></div>", unsafe_allow_html=True)
