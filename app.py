...
if selected:
    ...
    if not prices_hist.empty:
        ...
        _last_excel_price = {t: float(v) for t,v in _last_row.items()
                             if v and str(v).strip() not in ("","0") and float(v) > 0}
    ...
    px_source_model = f"Excel historico {len(prices_hist)} meses"
