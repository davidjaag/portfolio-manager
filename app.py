...
def load_prices(sheet_id: str, port_name: str) -> pd.DataFrame:
    ...
    Replica exactamente MATLAB:
      filas_validas = all(~isnan(Precios), 2)
    Solo incluye filas donde TODOS los activos tienen precio > 0.
    Esto garantiza que el modelo use los mismos datos que MATLAB
    sin perder filas por dropna() posterior.
    """
    gc = get_gc()
    if gc is None: return pd.DataFrame()
    try:
        ...
        # Identificar columnas de activos (excluir Fecha/Date)
        fecha_idx = None
        ...
        for row in data[1:]:
            ...
            # Guardar fecha para diagnóstico
            if fecha_idx is not None and fecha_idx < len(row):
                fecha_raw = str(row[fecha_idx]).strip()
                if fecha_raw:
                    vals["__fecha__"] = fecha_raw

            # Leer precio de cada activo
            valid = True
            for j, tkr in asset_hdrs:
                ...
                try:
                    ...
                    if v > 0:
                        ...
                        valid = False; break  # precio 0 o negativo = dato faltante
                except Exception:
                    valid = False; break

            # MATLAB: filas_validas = all(~isnan(Precios), 2)
            # Solo agregar si TODOS los activos tienen precio válido
            if valid and len([k for k in vals if not k.startswith("__")]) == n_assets:
                rows.append(vals)
