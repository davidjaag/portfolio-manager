...
.banner-greeting{font-size:17px;color:rgba(255,255,255,0.88);margin-top:10px;font-weight:500;letter-spacing:0.01em;}
...
@media(max-width:480px){
    .app-banner{flex-direction:column;align-items:flex-start;gap:10px;padding:16px;border-radius:12px;}
    .banner-right{text-align:left;width:100%;display:flex;align-items:center;gap:8px;}
...
if not st.session_state.authenticated:
    ...
    @media(max-width:640px){
        /* Colapsar columnas espaciadoras — hacer el panel central ancho completo */
        [data-testid="stAppViewContainer"] > section > div > div > div
            > div[data-testid="stHorizontalBlock"] > div:nth-child(1),
        [data-testid="stAppViewContainer"] > section > div > div > div
            > div[data-testid="stHorizontalBlock"] > div:nth-child(3){
            flex:0 0 0px !important;min-width:0 !important;overflow:hidden !important;padding:0 !important;
        }
        [data-testid="stAppViewContainer"] > section > div > div > div
            > div[data-testid="stHorizontalBlock"] > div:nth-child(2){
            flex:1 1 100% !important;min-width:0 !important;
        }
        .gpanel{padding:22px 18px;border-radius:16px;}
...
with st.sidebar:
    ...
    with st.expander("⚙ Configuración Google Sheets", expanded=False):
        if use_sheets:
            _src = "propio" if _own_sheet else "compartido"
            st.success(f"✓ Conectado ({_src})")
        elif sheet_id:
    ...
    # TC desde Yahoo Finance — informativo para todos, editable solo para admin
    _tc_yf = 20.50
    if YF_AVAILABLE:
        try:
            _tc_live = getattr(yf.Ticker("MXN=X").fast_info, "last_price", None)
            if _tc_live and 15 < float(_tc_live) < 30:
                _tc_yf = round(float(_tc_live), 2)
        except: pass
    ...
    if _is_admin_sb:
        ...
        thresh_rebal  = 0.05
        st.markdown(
            f"<div style='background:#1a3a5c22;border:1px solid #1a3a5c44;border-radius:8px;"
            f"padding:10px 14px;font-size:12px;color:#e8f0f8;margin-bottom:4px;'>"
            f"<span style='color:#c9a227;font-size:10px;font-weight:700;letter-spacing:.08em;'>"
            f"USD / MXN</span><br>"
            f"<span style='font-size:20px;font-weight:800;'>{_tc_yf:.2f}</span>"
            f"<span style='font-size:10px;color:#6b7c93;margin-left:6px;'>tiempo real</span></div>",
            unsafe_allow_html=True
        )
...
<div class="app-banner">
    ...
    <div class="banner-right">
        ...
        <div class="banner-greeting">{_saludo}, {_nombre_display}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    ...
    if not _is_admin:
        ...
        # Saludo personalizado según hora
        _hora_inv = datetime.now().hour
        _saludo_inv = 'Buenos días' if 6 <= _hora_inv < 13 else 'Buenas tardes' if 13 <= _hora_inv < 20 else 'Buenas noches'
        _nombre_inv = user_data.get('name', current_user)
        st.markdown(f"""
        <div style='text-align:right;font-size:16px;color:#1a3a5c;margin-bottom:12px;
                    font-weight:500;letter-spacing:0.01em;'>
            {_saludo_inv}, <span style='color:#c9a227;font-weight:700;'>{_nombre_inv}</span>
        </div>""", unsafe_allow_html=True)
