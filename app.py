        ...
        for i, row in price_df.iterrows():
            ...
            for tkr in price_df.columns:
                ...
                if qty > 0 and price_now > 0:
                    portfolio_value += qty * price_now  # USD — no tc
...
def calc_alerts(positions_df, target_weights, tc, monthly_contrib,
    ...
    if total_mxn == 0:
        ...
        df["Alerta"]              = "OK"
        df["MXN Diferencia"]      = 0.0
        df["Compra Sugerida MXN"] = 0.0
        df["Acciones a Comprar"]  = 0.0
...
if use_sheets and sheet_id:
    ...
    if ops_raw is not None and not ops_raw.empty:
        # Filtrar por usuario si existe la columna
        if "Usuario" in ops_raw.columns:
            ops_df = ops_raw[ops_raw["Usuario"].astype(str).str.lower() == username.lower()].copy()
            if ops_df.empty:
                ops_df = ops_raw  # fallback: mostrar todo si no hay match
        else:
            ops_df = ops_raw
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
                    ...
                    if q_ticker:
                        if use_sheets:
                            ok = save_row(sheet_id, f"Ops_{selected_port}",
                                          [str(q_fecha), q_ticker, q_tipo, q_cant, q_precio, q_com, q_tc])
                            if ok:
    ...
    with st.form("new_operation", clear_on_submit=True):
        ...
        if submitted:
            if ticker:
                ...
                if use_sheets:
                    row_list = [str(fecha), ticker, tipo, cantidad, precio, comision, tc_op, username]
    ...
    if not ops_df.empty:
        ...
        <div class="table-wrap"><table class="styled-table">
            <thead><tr>
                <th>Fecha</th><th>Ticker</th><th>Tipo</th><th>Cantidad</th><th>Usuario</th>
...
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    ...
    st.markdown("<div style='font-size:12px;color:#6b7c93;margin-bottom:20px;'>Resumen rápido de todos los usuarios y sus portafolios.</div>", unsafe_allow_html=True)
    all_ports = get_demo_portfolios()
    for uname, udata in USERS.items():
        uports = udata.get("portfolios") or list(all_ports.keys())
        if not uports: continue
        st.markdown(f"""<div style='background:#1a3a5c;color:white;border-radius:12px 12px 0 0;
             padding:10px 18px;margin-top:20px;display:flex;align-items:center;gap:12px;'>
          <div style='background:#c9a227;border-radius:50%;width:36px;height:36px;display:flex;
               align-items:center;justify-content:center;font-weight:800;font-size:14px;color:#1a3a5c;'>
            {udata.get("avatar","?")}</div>
          <div><div style='font-weight:700;font-size:15px;'>{udata.get("name",uname)}</div>
            <div style='font-size:11px;color:#c9d6e3;'>{udata.get("role","investor").upper()} · {len(uports)} portafolio(s)</div>
          </div></div>""", unsafe_allow_html=True)
        for pname in uports:
                ...
                try:
                    ...
                    if _raw is not None and not _raw.empty:
                        if "Usuario" in _raw.columns:
                            u_ops = _raw[_raw["Usuario"].astype(str).str.lower() == uname.lower()].copy()
                            if u_ops.empty: u_ops = _raw
                        else:
                            u_ops = _raw
                except: pass
            if u_ops.empty: u_ops = get_demo_operations(pname)
            u_pos = calc_positions(u_ops, {}, tc_actual)
            ...
            c1,c2,c3,c4=st.columns(4)
            with c1:
                st.markdown(f"""<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>📁 {pname}</div>
                  <div style='font-size:20px;font-weight:800;color:#1a3a5c;'>${total_mxn_u:,.0f}</div>
                  <div style='font-size:11px;color:#6b7c93;'>MXN · ${total_usd_u:,.0f} USD</div></div>""", unsafe_allow_html=True)
            with c2:
                st.markdown(f"""<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>📋 Operaciones</div>
                  <div style='font-size:20px;font-weight:800;color:#1a3a5c;'>{n_ops}</div>
                  <div style='font-size:11px;color:#6b7c93;'>registradas</div></div>""", unsafe_allow_html=True)
            with c3:
                st.markdown(f"""<div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>⚡ Sharpe Ratio</div>
                  <div style='font-size:20px;font-weight:800;color:{sharpe_color};'>{sharpe_u:.2f}</div>
                  <div style='font-size:11px;color:#6b7c93;'>Ret {ret_u*100:.1f}% · Vol {vol_u*100:.1f}%</div></div>""", unsafe_allow_html=True)
            with c4:
                st.markdown(f"""<div style='background:white;border:1px solid #e8eef5;border-radius:0 0 12px 0;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>🎯 Pesos objetivo</div>
                  <div style='font-size:11px;color:#1a3a5c;line-height:1.8;'>{wts_str}</div></div>""", unsafe_allow_html=True)
