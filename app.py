...
# Enriquecer solo con tickers de ops de ESTE portafolio
# (ops_df ya fue filtrado por selected_port y usuario)
if not ops_df.empty and "Precio_USD" in ops_df.columns:
    ...
    if prices is None:
        prices = _ops_prices
        prices_source = "Último precio registrado"
    else:
        for _t, _p in _ops_prices.items():
            if _t not in prices or prices.get(_t, 0) == 0:
                prices[_t] = _p
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with c1:
            ...
            <div class="kpi-card anim-fadeinup anim-delay-1">
                ...
                <div class="kpi-value">${total_mxn:,.0f}</div>
                <div class="kpi-sub">MXN · TC {tc_actual:.2f}</div>
            </div>""", unsafe_allow_html=True)
