...
def calc_risk_metrics_bloomberg(log_ret_df, weights, rf=0.0264):
    try:
        ...
        if len(R) < 12: return {}
        mu_ann  = R.mean(axis=0) * 12
        cov_ann = np.cov(R.T, ddof=1) * 12
        ...
        max_dd  = float(np.min((cum - mx) / mx))
        var_95  = float(np.percentile(pr, 5)); mask = pr < var_95
        cvar_95 = float(pr[mask].mean()) if mask.any() else var_95
        roll_v  = float(pd.Series(pr).rolling(6).std().iloc[-1] * np.sqrt(12)) if len(pr) >= 6 else None
        return {"vol_annual": vol_p, "ret_annual": ret_p, "sharpe": sharpe,
                "max_dd": max_dd, "total_return": float(cum[-1] - 1),
                "var_95": var_95, "cvar_95": cvar_95, "rolling_vol": roll_v, "n_months": len(pr)}
    except: return {}
        ...
        for pname in uports:
            ...
            if uid_sheet and use_sheets:
                try:
                    ...
                    if _raw is not None and not _raw.empty: u_ops = _raw
                except: pass
