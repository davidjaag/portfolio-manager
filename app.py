    ...
    try:
        ...
        for tab in [f"Precios_{portfolio_name}", "Precios"]:
            try:
                ...
                headers = data[0]
                rows    = data[1:]
                # Construir DataFrame — ignorar filas vacías del Sheet
                price_rows = []
                for row in rows:
                ...
                # Log-retornos: diff(log(P)) — idéntico a MATLAB
...
def calc_risk_metrics_bloomberg(log_ret_df, weights, rf=0.0264):
    """Idéntico a MATLAB: mean*12, cov(ddof=1)*12, Sharpe=(ret-rf)/vol"""
    try:
...
with st.sidebar:
    ...
    # Google Sheets — conexión automática con sheet_id del usuario
    ...
    _ret_df_raw = None  # siempre inicializar antes del bloque if
...
if _ret_df_raw is not None and target_wts:
    _bbg_rm = calc_risk_metrics_bloomberg(
        _ret_df_raw, target_wts, rf=port_data.get("rf", 0.0264))
...
if page in ("📊 Dashboard", "📊 Mi Portafolio"):
    ...
    if not _is_admin:
        ...
        if not YF_AVAILABLE:
            st.warning("Instala yfinance: `pip install yfinance`")
        else:
...
if page == "🔭 QuickView" and user_data.get("role") == "admin":
    st.markdown("<div class='section-title'>🔭 QuickView — Vista de todos los portafolios</div>",
                unsafe_allow_html=True)
    st.markdown(
        "<div style='font-size:12px;color:#6b7c93;margin-bottom:20px;'>"
        "Resumen rápido de todos los usuarios y sus portafolios.</div>",
        unsafe_allow_html=True)
    ...
    for uname, udata in USERS.items():
        ...
        <div style='background:#1a3a5c;color:white;border-radius:12px 12px 0 0;
          ...
          <div style='background:#c9a227;border-radius:50%;width:36px;height:36px;
               display:flex;align-items:center;justify-content:center;
               font-weight:800;font-size:14px;color:#1a3a5c;'>
            {udata.get("avatar","?")}
          </div>
          <div>
        ...
        for pname in uports:
            ...
            with c1:
                st.markdown(f"""
                <div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
                  ...
                  <div style='font-size:11px;color:#6b7c93;'>MXN · ${total_usd_u:,.0f} USD</div>
                </div>""", unsafe_allow_html=True)
            with c2:
                st.markdown(f"""
                <div style='background:white;border:1px solid #e8eef5;padding:14px 16px;'>
            ...
            with c4:
                st.markdown(f"""
                <div style='background:white;border:1px solid #e8eef5;border-radius:0 0 12px 0;padding:14px 16px;'>
                  <div style='font-size:11px;color:#6b7c93;font-weight:700;margin-bottom:4px;'>🎯 Pesos objetivo</div>
                  <div style='font-size:11px;color:#1a3a5c;line-height:1.8;'>{wts_str}</div>
                </div>""", unsafe_allow_html=True)
